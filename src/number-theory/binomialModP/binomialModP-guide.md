...
**핵심 아이디어**: "소수 모듈러에서 이항계수는 팩토리얼 역원으로, 큰 n에서는 Lucas 정리로 자릿수별 곱으로 분해한다"

$\binom{n}{k} \bmod p$를 직접 구하면 $n!$이 천문학적으로 크고, $n \geq p$이면 $n! \bmod p = 0$이 돼 역원조차 구할 수 없다. $n < p$인 경우에는 페르마의 소정리($a^{-1} \equiv a^{p-2} \pmod{p}$)로 팩토리얼 역원을 $O(\log p)$에 구할 수 있고, $n \geq p$인 경우에는 Lucas 정리로 $n$과 $k$를 $p$진수 자릿수별로 분해해 각 자릿수의 이항계수를 곱한다.

**풀이 구조**
1. $k < 0$ 또는 $k > n$이면 0 반환 (경계 처리)
2. $n < p$이면 팩토리얼 배열 전처리 후 페르마 역원으로 $\binom{n}{k} \bmod p$ 계산
3. $n \geq p$이면 Lucas 정리 적용: $\binom{n}{k} \equiv \binom{n \bmod p}{k \bmod p} \cdot \binom{\lfloor n/p \rfloor}{\lfloor k/p \rfloor} \pmod{p}$로 재귀 분해
4. 재귀 호출이 모두 $n < p$ 기저 사례에 도달하면 결과를 곱해 반환

**조건**: $p$는 반드시 소수여야 한다. 페르마의 소정리와 Lucas 정리 모두 소수 모듈러를 전제한다.

**대표 예시**: $\binom{7}{3} \bmod 5$ 계산
$n = 7 \geq p = 5$이므로 Lucas 적용: $n \bmod 5 = 2$, $k \bmod 5 = 3$, $\lfloor 7/5 \rfloor = 1$, $\lfloor 3/5 \rfloor = 0$. $\binom{2}{3} = 0$이므로 전체 결과는 $0 \cdot \binom{1}{0} = 0$이다. $k_i > n_i$인 자릿수가 있으면 즉시 0이 된다는 Kummer 정리적 직관을 보여주는 예시다.

**언제 쓰나**
조합 수나 경우의 수를 소수 $p$로 나눈 나머지를 구해야 할 때, 특히 $n$이 $p$보다 크거나 매우 커서 일반 팩토리얼 계산이 불가능한 경쟁 프로그래밍 문제에서 사용한다.
