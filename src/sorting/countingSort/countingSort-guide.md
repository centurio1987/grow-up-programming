...
**핵심 아이디어**: "값의 범위가 제한된 정수라면, 비교 없이 빈도만 세어도 정렬할 수 있다."

비교 기반 정렬은 아무리 최적화해도 $O(N \log N)$ 이상이 든다. 그러나 원소가 $[0, k]$ 범위의 정수라면, "0이 몇 개, 1이 몇 개, …, k가 몇 개"를 세는 것만으로 정렬 결과를 직접 만들 수 있다. 비교 자체가 불필요해지는 것이다.

**풀이 구조**
1. 크기 $k+1$의 빈도 배열 $C$를 0으로 초기화
2. 입력 배열을 순회하며 각 값의 빈도를 $C$에 집계
3. $v = 0$부터 $k$까지 $C[v]$개만큼 $v$를 출력 배열에 붙여 넣음
4. 완성된 배열 반환 (안정 정렬이 필요하면 누적합 + 역방향 배치 방식 사용)

**조건**: 원소가 정수이고 값의 범위 $k$가 $N$보다 훨씬 크지 않아야 효율적. $k \gg N$이면 오히려 손해.

**대표 예시**: 학생 점수(0~100점) 배열 정렬
점수가 0~100 범위로 제한되므로 크기 101의 빈도 배열 하나로 수십만 명의 점수를 O(N)에 정렬할 수 있다. 비교 기반 정렬로는 도달할 수 없는 선형 시간이다.

**언제 쓰나**
정렬할 원소가 정수이고, 값의 범위 $k$가 입력 크기 $N$과 비슷하거나 더 작을 때 사용한다. 나이, 점수, 우편번호처럼 제한된 범위의 정수 데이터에 적합하다.
