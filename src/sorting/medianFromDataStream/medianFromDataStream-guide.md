...
**핵심 아이디어**: "정렬 배열을 하위 절반과 상위 절반으로 쪼개 각각 힙으로 관리하면, 중앙값이 항상 두 힙의 루트에 있다."

중앙값을 구하기 위해 전체를 재정렬할 필요는 없다. 정렬된 배열의 절반을 경계로 나누면 중앙값은 하위 절반의 최댓값 또는 상위 절반의 최솟값만으로 결정된다. max-heap(하위 절반)과 min-heap(상위 절반)을 유지하면 삽입마다 $O(\log N)$, 중앙값 조회는 $O(1)$이다.

**풀이 구조**
1. `lo`(max-heap, 하위 절반)와 `hi`(min-heap, 상위 절반) 초기화
2. `addNum`: `num ≤ lo.top()`이면 `lo`에, 아니면 `hi`에 삽입
3. 크기 불변식 복원: `lo`가 2개 이상 초과하면 `lo.top()`을 `hi`로, `hi`가 더 크면 `hi.top()`을 `lo`로 이동
4. `findMedian`: `lo.size() > hi.size()`이면 `lo.top()`, 같으면 `(lo.top() + hi.top()) / 2`

**조건**: 두 불변식 유지 필수 — 값 불변식 `lo.top() ≤ hi.top()`, 크기 불변식 `|lo.size() - hi.size()| ≤ 1`.

**대표 예시**: 실시간 센서 데이터 스트림의 중앙값 모니터링
초당 수천 개의 온도 데이터가 들어올 때, 매번 중앙값을 계산해야 한다. 두 힙 구조로 삽입마다 $O(\log N)$, 중앙값 조회는 $O(1)$이므로 스트리밍 처리에 적합하다.

**언제 쓰나**
원소가 동적으로 추가되는 스트림에서 중앙값을 반복 조회해야 할 때 사용한다. 배열 전체를 정렬하지 않고 삽입과 조회 각각의 비용을 균형 있게 낮추는 패턴이다.
