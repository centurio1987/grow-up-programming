...
고정 입력 `A = [1, 1, 1, 2, 2, 3]`, `k = 2`에 Bucket Sort 방식을 적용한다. 빈도를 집계한 뒤 버킷 배열 `B`(인덱스 = 빈도)에 고유값을 분류하고, 높은 빈도 버킷부터 역순으로 k개를 수집한다. 위 array 패널은 버킷 배열 B를 인덱스(빈도)별로 보여주며, 빨간색(`highlight`)은 현재 수집 중인 버킷 인덱스, 아래 keyValue 패널은 빈도 맵과 현재 결과 스냅샷이다.

실제 반환값은 `[1, 2]` (빈도 1=3회, 2=2회) 이며, 시뮬레이션 마지막 프레임의 result와 일치한다.
...
export const steps = [
  {
    title: "초기화",
    detail: "입력 A = [1, 1, 1, 2, 2, 3]. 빈도 집계 시작.",
    array: ["B0", "B1", "B2", "B3"],
    entries: [
      { label: "freq", value: "{}" },
      { label: "result", value: "[]" },
  ...
  {
    title: "버킷 분류",
    detail: "값 1(빈도3)->B[3], 2(빈도2)->B[2], 3(빈도1)->B[1].",
    array: ["B0:[]", "B1:[3]", "B2:[2]", "B3:[1]"],
    entries: [
      { label: "freq", value: "1->3, 2->2, 3->1" },
      { label: "result", value: "[]" },
    ],
  ...
  {
    title: "수집 i=2",
    detail: "B[2]=[2] 수집. result=[1, 2], k=2 달성 → 즉시 반환.",
    array: ["B0:[]", "B1:[3]", "B2:[2]", "B3:[1]"],
    highlight: [2],
    marked: [3],
    entries: [
