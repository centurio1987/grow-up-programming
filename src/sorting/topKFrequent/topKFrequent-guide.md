...
**핵심 아이디어**: "상위 k개만 필요하다면 전체를 정렬할 필요 없이, 크기 k의 힙으로 후보를 관리하거나 빈도를 버킷으로 분류하면 된다."

전체 고유값을 빈도순으로 정렬하면 $O(N \log N)$이지만, 상위 k개만 원한다면 낭비다. min-heap으로 "지금까지의 상위 k개 후보"를 유지하면 $O(N \log k)$로 줄어든다. 빈도 범위가 $[1, N]$으로 제한된다는 사실을 이용하면 Bucket Sort로 $O(N)$까지 줄일 수도 있다.

**풀이 구조 (Bucket Sort 방식)**
1. HashMap으로 각 원소의 빈도 집계 — $O(N)$
2. 크기 $N+1$의 버킷 배열 $B$ 생성. 빈도 $f$인 고유값을 $B[f]$에 넣음
3. $i = N$부터 $1$까지 역순으로 $B[i]$의 원소를 수집
4. $k$개가 모이면 즉시 반환

**조건**: k가 고유값 개수 이하여야 함. Bucket Sort 방식은 빈도 범위가 $[1, N]$으로 제한됨을 전제로 $O(N)$ 달성.

**대표 예시**: 검색 로그에서 가장 많이 검색된 키워드 상위 k개 추출
수백만 건의 검색 로그에서 빈도를 집계한 뒤, 전체 정렬 없이 Bucket Sort로 상위 k개 키워드를 $O(N)$에 추출할 수 있다.

**언제 쓰나**
빈도 집계 후 상위 k개를 선택하는 "top-k by frequency" 패턴 문제에서 사용한다. k가 충분히 작으면 min-heap으로, k와 N이 비슷한 규모이거나 $O(N)$이 필요하면 Bucket Sort 방식을 선택한다.
