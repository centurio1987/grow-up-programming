 ...
 *
 * **꼬리를 새 문자열로 만들어 쪼개는 구현을 잡는 시나리오도 여기가 아니라 저쪽에 있다.** 처음
 * 스위트가 그 자리를 재지 않아 그 구현이 다섯 시나리오를 전부 통과했고(`./radixTree.ts` 헤더), 이
 * 파일에 더하면 스위트가 두 벌이 되므로 정본 스위트 끝에 「곁에 긴 낱말」 둘로 더했다(KAN-026 S20).
 * 걸리는 자리는 자기시험(`src/data-structures/_contract/runContract.radixTree.test.ts`)이 고정한다.
 *
