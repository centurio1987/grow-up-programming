 ...
 *
 * **축3 계측(`__cost`).** 세는 단위는 *"칸 하나를 지나가거나 문자 하나를 견줄 때마다 1"* 이다 —
 * **읽기와 쓰기를 따로 세지 않는다.** 축3은 절대 카운트가 아니라 성장률을 보므로 상수 배수가
 * 판정에 들어오지 않기 때문이다(§규약2 계측 단위). `__cost` 는 계약이 아니라 정본의 의무다(불변 사실 23)
 * — 계약 헤더에 적지 않고 학습자 스텁에 요구하지 않는다.
 *
