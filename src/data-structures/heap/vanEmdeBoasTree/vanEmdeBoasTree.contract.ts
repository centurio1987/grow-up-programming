 ...
 * 있으므로 시나리오 **여덟**을 **칸 넷**으로 가른다 — **파라미터 둘 × 사다리를 읽는 방식 둘**이고,
 * 한 칸에 시나리오가 여럿인 것은 그 칸에서 `covers` 가 갈리기 때문이다.
 *
 * | 사다리가 오르는 것 | 고정하는 것 | bound | 시나리오 | 잡는 계열 |
 * |---|---|---|---|---|
 * | 담긴 원소 수 n(4 배씩) | 우주 $2^{16}$ | `O(1)` | 셋 — 넣기 · 지우기 · 조회 | 원소 수에 비례하는 계열(정렬 배열) |
 * | 우주 u(4 배씩) | 담긴 원소 한둘 | `O(1)` | 둘 — 조회 · 갱신 | 우주 크기의 거듭제곱에 비례하는 계열(비트 배열 · 제곱근 묶음) |
 * | 우주의 **비트 수**(두 배씩) | 담긴 원소 한둘 | `O(log n)` | **둘 — 조회 · 갱신**(`S4` 가 갱신을 붙였다) | 우주의 로그로 어기는 계열(비트 하나씩 내려가는 트라이) |
 * | 담긴 수의 **비트 수**(두 배씩) | 우주 $2^{16}$ | `O(1)` | 하나 — 조회 | 원소 수의 로그로 어기는 계열(비교로만 견주는 균형 트리) |
 *
 * **앞의 두 칸은 bound 가 `O(1)` 이다.** 앞쪽은 상한에 n 이 없으니 그대로이고, 뒤쪽은 $\log\log u$ 가
 * 그 사다리에서 비율 1.06~1.08 이라 `O(1)` 구간(0.70~1.30) 한복판에 놓이기 때문이다 — 이 정본은 u =
 * $2^{10}$ · $2^{12}$ · $2^{14}$ 에서 내려가는 깊이가 넷으로 같다. **그러므로 그 다섯이 판정하는 것은
 * 「u 의 로그 로그」가 아니라 「u 의 어떤 거듭제곱보다도 느리게 자란다」이고**, 로그 인수만큼 어기는
 * 계열(비트 하나씩 내려가는 트라이 · 비교 기반 균형 트리)은 그 다섯을 전부 통과한다(불변 사실 53·62).
 *
 * **뒤의 두 칸(시나리오 셋)이 그 자리를 잡으려고 스위트 끝에 붙은 한 벌이다.** 사다리의 세 점을
 * 크기가 아니라 **비트 수**로 읽으면 로그 인수가 비율 2.0 으로, 로그 로그가 1.43 · 1.30 으로 벌어져
 * 둘이 다른 구간에 선다. 하네스(`_contract/runContract.ts` · `_contract/judge.ts`)는 그대로이고 읽기만
 * 시나리오 안에서 바뀐다(`bitsOf`). **우주 비트 칸에 시나리오가 둘인 것이 `S4` 가 고친 자리다** —
 * 계약의 필요충분조건이 「이웃 찾기와 갱신이 **함께**」를 요구하는데 조회만 재면 넣기만 $O(\log u)$ 인
 * 계열이 여덟 중 일곱을 지나간다. 담긴 수 쪽은 조회 하나로 둔다(헤더 「검사 공백」의 n 쪽 문단).
 * 남는 공백($\sqrt{\log u}$ 계열)은 헤더 「검사 공백」이 적는다.
 *
 * **담긴 수 비트 사다리(일곱째)는 셋째와 우주까지 같다.** 그래서 그 사다리의 끝점이 셋째 시나리오의
 * 가운데 점과 **같은 입력**이고, 여섯 대상의 걸음이 거기서 전부 맞아떨어진다(자기시험이 고정한다).
 * 「바뀐 것은 사다리를 읽는 방식 하나다」가 말이 아니라 대조되는 수치인 자리다. 여섯째 · 여덟째도
 * 넷째 · 다섯째와 입력의 모양이 같아 같은 대조를 진다.
 */
 ...
 *
 * 마지막 세 시나리오가 이 읽기를 쓴다. 사다리의 n 을 원소 수나 우주 크기로 그대로 읽으면 로그
...
/**
 * 우주를 비트 수로 올리는 **두 시나리오**(조회 · 갱신)의 밑 비트 수 — 5 · 10 · 20 비트
 * (u = $2^5$ · $2^{10}$ · $2^{20}$). 둘이 같은 값을 쓰므로 사다리가 같고 `covers` 만 갈린다.
 */
...
export const vanEmdeBoasTreeContract: ContractSpec<UniverseSite, Model> = {
  ...
  scenarios: [
    ...
    },
    {
      ...
      // 32,772 — 조회 시나리오 셋은 전부 통과했다). **갱신이 $O(\log u)$ 인 계열은 여기서 통과한다**
      // — 비트 하나씩 내려가는 트라이가 24 · 28 · 32(비율 1.17 · 1.14)다. 그 자리는 여덟째 시나리오
      // (우주를 비트 수로 읽는 갱신 사다리)가 잡는다.
      run: (impl, n, ctx) => {
        impl.reset(n);
        impl.insert(0);
        for (let round = 0; round < ROUNDS; round++) {
    ...
    },
    {
      ...
      // 13 · 13 · 14 로 통과한다. 그 자리는 일곱째 시나리오다. 비율 1.41 인 $\sqrt{\log u}$ 계열은
      // 이 구간 안이라 여전히 통과한다(산술 — 헤더 「검사 공백」).
      //
      // **갱신 쪽 로그도 이 사다리가 못 잡는다** — `covers` 에 넣기 · 지우기가 없다. 그 자리는
      // 여덟째 시나리오이고, 사다리는 이것과 같고 `covers` 만 갈린다(`S4`).
      run: (impl, n, ctx) => {
    ...
    },
    {
      ...
      bound: "O(log n)",
      adversarial: true,
      // **우주를 비트 수로 키운다 — 비트 수 5 · 10 · 20, 키 0 을 둔 채 우주의 마지막 키를 넣었다
      // 지우기.** 위 다섯째(4 배 갱신) 시나리오와 입력의 모양이 같고 **사다리를 읽는 방식만 다르다** —
      // 여섯째가 조회 쪽에 한 것을 갱신 쪽에 그대로 한 자리다. **갱신의 로그 인수를 재는 것은 이
      // 시나리오뿐이다**(`S4`). 여섯째 · 일곱째의 `covers` 에는 넣기 · 지우기가 없어서, 이웃 찾기는
      // 로그 로그인데 **넣기만** $O(\log u)$ 인 구현 — 요약과 클러스터 양쪽으로 재귀하는 원형 판
      // 넣기 — 이 그 일곱을 전부 지나갔다. 필요충분조건이 요구하는 「이웃 찾기와 갱신이 **함께**」의
      // 뒤쪽 절반을 여기서 잰다.
      //
      // 비트 하나씩 내려가는 트라이가 14 · 24 · 44($r$ = 1.71 · 1.83)로 걸리고
      // (`_contract/_fixtures/bitTrieIntegerSet.ts` — 4 배 갱신 사다리에서는 24 · 28 · 32 라 비율이
      // 1.17 · 1.14 로 통과했다), 이 정본은 15 · 17 · 19(1.13 · 1.12)다. 칸을 훑는 계열은 33 · 1,025 ·
      // 1,048,577 로 더 크게 걸린다. 통과하는 셋 — 결정론 트립 7 · 7 · 9(담긴 키가 둘뿐이라 원소 쪽
      // 로그가 안 보인다 · 일곱째가 잡는다) · 크기 순 배열 5 · 5 · 5 · 넣기를 쌓아 두는 계열
      // 23 · 27 · 31.
      //
      // bound 가 `O(log n)` 인 까닭은 여섯째와 같다 — 사다리를 오르는 파라미터가 우주의 비트 수 $b$
      // 라 계약의 $\log\log u$ 가 그 자리에서 $\log b$ 다. **적대적이다** — 우주의 양 끝을 겨눈 고른
      // 입력이라 칸을 한 칸씩 훑는 계열의 최악이고(다섯째 · 여섯째가 참인 것과 같은 까닭), 사다리를
      // 읽는 방식만 바꾼 일곱째가 거짓인 것과 갈리는 자리가 여기다. 다섯째와 행 · 한정자 · 적대
      // 여부가 같아 자기시험 이름에 차례 `#2` 가 붙는다 — 하네스 시험 이름은 상한이 달라 겹치지 않는다.
      run: (impl, n, ctx) => {
        const universe = 2 ** bitsOf(n, UNIVERSE_LADDER_BASE);
        impl.reset(universe);
        impl.insert(0);
