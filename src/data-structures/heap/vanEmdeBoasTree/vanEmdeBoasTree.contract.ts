 ...
 * | 담긴 수의 **비트 수**(두 배씩) | 우주 $2^{22}$ | `O(1)` | 원소 수의 로그로 어기는 계열(비교로만 견주는 균형 트리) |
 *
...
/**
 * 담긴 수를 비트 수로 올리는 시나리오가 고정하는 우주. 끝점에서도 담긴 키가 우주의 0.1% 아래라
 * 이 사다리가 움직이는 것은 담긴 수 하나다.
 */
const WIDE_UNIVERSE = 2 ** 22;
...
export const vanEmdeBoasTreeContract: ContractSpec<UniverseSite, Model> = {
  ...
  scenarios: [
    ...
    },
    {
      ...
      bound: "O(1)",
      adversarial: true,
      // **담긴 수를 비트 수로 키운다 — 우주 $2^{22}$ 고정, 담긴 수 $2^3$ · $2^6$ · $2^{12}$.** 위 셋째
      // 시나리오와 채우기·조회가 같고(네 칸마다 키 하나를 무작위 자리에, 조회는 그 범위 안에서 무작위)
      // **사다리를 읽는 방식만 다르다.** 원소 쪽 로그로 어기는 계열을 잡는 자리다 — 비교로만 견주는
      // 트립이 15 · 39 · 66($r$ = 2.60 · 1.69)으로 걸리고
      // (`_contract/_fixtures/hashPriorityTreapIntegerSet.ts`), 이 정본은 33 · 34 · 34 다. 우주 쪽
      // 로그로 어기는 트라이는 담긴 수와 무관하므로 75 · 78 · 78 로 통과한다 — 위 시나리오가 잡는다.
      //
      // **적대적으로 적는 것은 사다리를 읽는 방식이 고른 것이기 때문이다.** 담긴 수를 네 배씩 올리면
      // 같은 계열이 통과하고(위 셋째 시나리오에서 63 · 66 · 81) 비트 수로 올려야 걸린다. 겸해서 이
      // 항목이 판정 이름을 가르는 자리다 — 행 · 한정자 · 상한이 같은 시나리오가 둘이면 이름이 겹친다
      // (§규약2 「성격 전환이 찾은 정본 스위트의 구멍은 정본 스위트 끝에 한 벌을 붙여 막는다」).
      //
      // **사다리 끝을 $2^{20}$ 에서 $2^{12}$ 로 낮춰 달았다.** 끝이 $2^{20}$ 이면 판정이 같고(정본
      // 34 · 34 · 34 · 트립 27 · 63 · 114(2.33 · 1.81) · 트라이 77 · 78 · 78) 계측이 3,735 만인데,
      // 크기 순 배열 fixture 가 원소 $2^{20}$ 개를 채우는 데 원소 수의 제곱이 들어 하네스 자기시험이
      // 그 계열을 이 시나리오에 태우지 못한다(§규약2 같은 절 — 결함 목록을 새 시나리오에 다시
      // 태운다). 낮춘 사다리는 계측 1,714 만이고 그중 대부분이 우주를 세우는 준비다.
      run: (impl, n, ctx) => {
        const count = 2 ** bitsOf(n, COUNT_LADDER_BASE);
        impl.reset(WIDE_UNIVERSE);
        const span = 4 * count;
