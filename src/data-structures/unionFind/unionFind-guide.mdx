...
`n = 5`로 초기화한 뒤 `union(0,1)`, `union(2,3)`, `union(1,2)`를 차례로 수행하고 마지막에 `connected(0,3)`을 조회한다. keyValue 패널은 `parent`와 `rank` 배열, 그리고 현재 집합 구성을 보여준다. 경로 압축과 랭크 기준 합치기가 적용된다.

마지막 `connected(0,3)`의 실제 반환값은 `true` (0, 1, 2, 3이 한 집합) 이며, 시뮬레이션 마지막 프레임과 일치한다.
