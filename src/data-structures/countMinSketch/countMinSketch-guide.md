...
**핵심 아이디어**: "정확한 빈도 대신, 여러 독립 행의 최솟값으로 과대추정을 최소화한다."

스트리밍 데이터에서 수백만 종류의 원소 빈도를 정확히 기록하려면 메모리가 감당이 안 된다. Count-Min Sketch는 $d \times w$ 크기의 작은 행렬만 유지하면서, 각 행에 독립 해시로 빈도를 누적하고 조회할 때 모든 행의 최솟값을 반환한다. 실제 빈도보다 크거나 같은 값이 항상 나오지만, 오차는 확률적으로 작게 제한된다.

**풀이 구조**
1. $d$개의 행, 각 행 $w$개의 카운터로 이루어진 행렬을 초기화한다
2. 원소 추가 시: $d$개의 독립 해시 위치에 각각 빈도를 더한다
3. 빈도 조회 시: $d$개의 해시 위치 카운터 중 최솟값을 반환한다(가장 오염이 적은 행 선택)

**조건**: 과대추정(상한 보장)은 허용하지만 과소추정은 절대 안 되는 환경, 실시간 스트림에서 고유 원소 수가 너무 많아 모두 저장할 수 없는 상황

**대표 예시**: 실시간 네트워크 트래픽 Heavy Hitter 탐지
초당 수백만 패킷이 들어오는 환경에서 특정 IP의 요청 횟수를 추적할 때, 모든 IP를 해시 맵에 저장하는 대신 Count-Min Sketch를 사용한다. 자주 등장하는 IP(Heavy Hitter)는 추정값이 실제와 거의 일치하며, 희귀 IP는 오차가 있어도 탐지 목적으로는 충분하다.

**언제 쓰나**
빈도 상위 원소(Heavy Hitter) 탐지, 실시간 로그 집계, 캐시 교체 정책 등 정확한 카운팅보다 메모리 효율이 더 중요하고 약간의 과대추정이 허용될 때 사용한다.
