# XOR 연결 리스트 — 이웃 주소 둘을 값 하나에 담기

## 파트 1 — 두 방향 읽기에서 저장 방식으로

### 전체 컨셉

XOR 연결 리스트가 다루는 것은 **뒤에만 원소를 붙이고, 전체를 앞에서부터와 뒤에서부터 두 방향으로 읽는 수열**입니다. 10, 0, 30을 차례로 붙이면 앞에서부터 읽은 결과는 `[10, 0, 30]`이고 뒤에서부터 읽은 결과는 `[30, 0, 10]`입니다.

```text
append(10)         [10]
append(0)          [10, 0]
append(30)         [10, 0, 30]
toArray()          [10, 0, 30]     앞에서부터 읽기 — 반환값
toArrayReverse()   [30, 0, 10]     뒤에서부터 읽기 — 반환값
```

이 수열을 저장하는 방법은 하나가 아닙니다. **동적 배열**은 붙인 순서대로 칸에 적고, **이중 연결 리스트**는 원소마다 노드를 만들어 이전 노드와 다음 노드를 따로 참조하며, **XOR 연결 리스트**는 그 두 참조를 XOR 한 값 하나만 노드에 둡니다. 세 방법 모두 마지막 원소의 위치를 기억하므로 붙일 때 전체를 순회하지 않습니다.

| 구현 | 이웃을 찾는 방법 | 원소가 많아질 때 생기는 작업 |
| --- | --- | --- |
| 동적 배열 | 인덱스에 1을 더하거나 뺀다 | 칸이 가득 차면 더 큰 배열로 복사 |
| 이중 연결 리스트 | 노드의 `prev`·`next` 참조를 읽는다 | 붙일 때 새 노드 생성 |
| XOR 연결 리스트 | 저장값에 직전 노드를 XOR 한다 | 붙일 때 새 노드 생성과 기존 마지막 노드 갱신 |

이름은 세 번째 구현에서 왔습니다. 이중 연결 리스트의 참조 두 개를 하나로 줄여 노드마다 메모리를 아끼려는 설계입니다. 다만 TypeScript에서는 메모리 주소를 정수로 꺼낼 수 없어서, 이 글의 TypeScript 구현은 주소 대신 노드 번호와 번호로 노드를 찾는 표를 씁니다. 그 표 때문에 오히려 메모리를 더 씁니다. 그래서 이 글은 XOR 연결 리스트를 주소를 직접 다룰 수 있는 Rust로 한 번 더 구현하고, 노드가 실제로 요청하는 바이트를 잽니다. 사용자에게 보이는 수열은 네 구현이 같고, 그 수열을 두 방향으로 따라가기 위해 저장하는 것이 다릅니다.

### 시작하기 전에 — 이미 알고 있어야 하는 것

배열 끝에 값을 추가하는 `push`, 객체가 다른 객체를 참조하는 방식, `Map`에 키로 값을 넣고 찾는 방법을 알고 있으면 됩니다. 앞뒤 참조를 둘 다 두는 리스트의 기본은 이중 연결 리스트 가이드(`linear/doublyLinkedList`)에, 배열 확장은 동적 배열 가이드(`linear/dynamicArray`)에 있습니다. 여기서는 필요한 만큼 작은 예로 다시 설명합니다.

세 번째 구현은 비트 XOR(`^`)을 씁니다. XOR은 두 수를 2진수로 놓고 자리마다 비트가 다르면 1, 같으면 0을 적는 연산입니다. 같은 수를 두 번 XOR 하면 0이 되고, 0과 XOR 하면 그대로이며, 계산 순서를 바꿔도 결과가 같습니다. 그래서 두 값을 XOR 한 결과와 그중 한 값을 알면 나머지 값을 되찾을 수 있습니다. 이 성질은 [`singleNumberXor` 가이드](../../../algorithms/bit-manipulation/singleNumberXor/singleNumberXor-guide.md)에서도 쓰입니다.
...
이 글에서 사용할 연산은 세 개입니다. 아래 표는 [이 프로젝트의 XOR 연결 리스트 명세](./xorLinkedList.ts)를 요약한 것입니다. 가운데에 넣거나 빼는 연산, 번호로 원소 하나를 읽는 연산, 원소 수를 읽는 연산은 이 명세에 없습니다.

<!--contract-summary-->

| 연산 | 의미 | 상한 | 한정자 |
| --- | --- | --- | --- |
| `append(value)` | `value`를 뒤 끝에 붙인다 | O(1) | 상각 |
| `toArray()` | 앞 끝에서 뒤 끝까지의 값 전부를 그 순서로 반환, 비어 있으면 빈 배열 | O(n) | 최악 |
| `toArrayReverse()` | 뒤 끝에서 앞 끝까지의 값 전부를 그 순서로 반환, 비어 있으면 빈 배열 | O(n) | 최악 |

n은 저장된 원소 수입니다. **최악 시간**이 O(n)이면 한 번의 호출이 원소 수에 비례하는 작업량 안에서 끝납니다. 두 순회는 원소 n개를 모두 돌려주므로 어떤 구현에서도 n에 비례하고, 여러 호출을 평균 내어 줄일 여지가 없습니다. **상각 시간**이 O(1)이면 빈 수열에서 시작한 m번의 붙이기에 총 O(m)의 작업이 필요합니다. 어떤 한 번은 오래 걸려도 되며, 입력이 무작위라는 가정은 필요하지 않습니다.
...
값은 정수이고 0이나 음수도 저장할 수 있습니다. 두 순회는 저장된 수열의 복사본을 돌려주므로 반환된 배열을 고쳐도 저장된 수열은 바뀌지 않습니다. 네 구현 모두 이 규칙을 따릅니다.
...
### 설계 상세 — 이웃 둘을 가리키는 데 값 하나면 될까

뒤에만 붙이고 전체를 두 방향으로 읽는 일은 배열 하나로 충분합니다. 붙인 순서대로 칸에 적으면 앞에서부터 읽기는 0번 칸부터, 뒤에서부터 읽기는 마지막 칸부터 차례로 읽는 일입니다.
...
원소를 연속된 칸에 두지 않고 노드로 이으면 사정이 달라집니다. 노드가 다음 노드만 가리키면 앞에서부터는 읽을 수 있지만, 뒤에서부터 읽으려면 이전 노드를 찾을 방법이 없습니다. 그래서 이중 연결 리스트는 노드마다 `prev`와 `next` 두 참조를 둡니다. 마지막 노드도 따로 기억해야 합니다. 마지막 노드를 모르면 붙일 때마다 첫 노드부터 끝까지 따라가야 하고, 원소 n개를 붙이는 동안 `0 + 1 + … + (n - 1) = n(n - 1) / 2`번 이동합니다. 원소 1,024개면 523,776번입니다.

```text
마지막 노드를 모르는 리스트에 붙이기
1번째 붙이기   이동 0
2번째 붙이기   이동 1
3번째 붙이기   이동 2
n번째 붙이기   이동 n - 1     합 n(n - 1) / 2 → O(n²)
```

그런데 두 방향으로 읽을 때 한 노드에서 실제로 필요한 것은 **지금 가는 방향의 다음 노드 하나**입니다. 직전에 읽은 노드는 이미 알고 있습니다. 이전 노드 번호와 다음 노드 번호를 XOR 한 값 하나만 저장해 두면, 거기에 직전 노드 번호를 XOR 해서 나머지 하나를 되찾을 수 있습니다. 이 조건을 받아들이면 세 저장 방식을 나란히 놓을 수 있습니다.

```text
동적 배열          붙인 순서대로 연속된 칸에 저장한다. 이웃은 인덱스 ±1이다.
이중 연결 리스트   노드마다 이전 노드와 다음 노드를 따로 가리킨다.
XOR 연결 리스트    노드마다 이전 id와 다음 id를 XOR 한 값 하나를 두고, 직전 노드로 다음 노드를 계산한다.
```

예를 들어 노드 번호 1, 2, 3에 값 10, 20, 30을 붙였다면 XOR 연결 리스트는 세 노드에 각각 `0 ^ 2 = 2`, `1 ^ 3 = 2`, `2 ^ 0 = 2`를 저장합니다. 이웃이 없는 자리는 번호 0으로 적습니다. 세 노드의 저장값이 모두 2라 이 값만 보고는 이웃을 알 수 없지만, 1번에서 2번으로 왔다면 `2 ^ 1 = 3`이 다음 노드이고 3번에서 2번으로 왔다면 `2 ^ 3 = 1`이 다음 노드입니다. 저장한 것은 배열과 다르지만 읽히는 수열은 같습니다.

### 수행으로 알아보는 자료구조 — 저장 방식을 바꾸며 두 방향으로 읽기

네 구현에서 공통으로 확인할 것은 붙인 순서와 두 순회의 반환값입니다. 먼저 빈 수열을 읽고, 10, 0, 30을 붙인 뒤 두 방향으로 읽어 보겠습니다. 둘째 값을 0으로 고른 것은 값 0이 이웃 없음을 뜻하는 번호 0과 같은 수여도 읽기가 멈추지 않는다는 것을 보이기 위해서입니다. XOR 연결 리스트에서는 붙일 때 바뀌는 노드가 어느 것인지와 잘못 쓰기 쉬운 두 줄을, Rust 구현에서는 번호가 주소로 바뀌면 무엇이 달라지는지를 추가로 살펴봅니다.

```text
공통 확인: toArray() → append(10) → append(0) → append(30) → toArray() → toArrayReverse()
추가 확인: 붙일 때 바뀌는 노드 · id 0과 prev 갱신 실수 · 주소를 XOR 하는 Rust 구현과 노드 크기
```

네 구현을 하나씩 만듭니다. 소절 번호의 앞자리가 구현이고 뒷자리가 그 구현 안의 순서입니다. 실수하기 쉬운 자리를 짚는 「짚고 가기」 소절은 그 구현의 소절 사이에 둡니다.

| 구현 | 소절 | 그 구현에서 따로 볼 것 |
| --- | --- | --- |
| 동적 배열 | 1-1 | 칸이 가득 찼을 때의 복사 |
| 이중 연결 리스트 | 2-1 | 빈 리스트에 붙일 때 `head` 설정 |
| XOR 연결 리스트 | 3-1 · 3-2 · 전체 코드 | 붙일 때 바뀌는 노드 둘, id 0과 `prev` 갱신 실수 |
| Rust XOR 연결 리스트 | 4-1 | 번호가 주소로 바뀌면 달라지는 것, 노드가 요청하는 바이트 |

#### 1-1. 동적 배열 — 붙인 순서가 곧 칸의 순서다

배열 `values` 하나에 붙인 순서대로 값을 넣습니다. `append(value)`는 `values.push(value)`입니다. `toArray()`는 배열을 복사해 돌려줍니다. 내부 배열을 그대로 돌려주면 호출한 쪽이 반환값을 고칠 때 저장된 수열까지 바뀌기 때문입니다. `toArrayReverse()`는 마지막 칸부터 0번 칸까지 읽어 새 배열에 담습니다. 저장된 배열을 뒤집지 않으므로 호출 뒤에도 `values`의 순서는 그대로입니다.
...
JavaScript 배열은 칸이 모자라면 더 큰 공간을 잡고 원소를 복사합니다. 언어 명세는 용량을 얼마나 늘리는지 정하지 않으므로, 이 글은 용량을 두 배씩 늘린다고 두고 셉니다. 복사가 일어나는 그 한 번의 붙이기는 원소 수만큼 복사하지만, m번 붙이는 총비용은 O(m)입니다. 그 계산은 비용 절에서 합니다.

#### 2-1. 이중 연결 리스트 — 마지막 노드를 기억하고 이웃 둘을 가리킨다

원소마다 노드 `{ value, prev, next }`를 만듭니다. 리스트는 첫 노드 `head`와 마지막 노드 `tail`을 기억합니다.
...
붙이기는 새 노드의 `prev`를 지금의 `tail`로 두고, 기존 `tail`의 `next`를 새 노드로 바꾼 뒤 `tail`을 새 노드로 옮기는 일입니다. 비어 있었다면 `head`도 새 노드를 가리켜야 합니다. 앞에서부터 읽기는 `head`에서 `next`를 따라가고, 뒤에서부터 읽기는 `tail`에서 `prev`를 따라갑니다. 읽기는 참조가 `null`일 때 멈추므로 값 0도 다른 값과 똑같이 읽힙니다.

| 호출 | 노드 연결 | head | tail |
| --- | --- | --- | --- |
| 시작 | 없음 | null | null |
| `append(10)` | `[10]` | 노드 10 | 노드 10 |
| `append(0)` | `[10] ↔ [0]` | 노드 10 | 노드 0 |
| `append(30)` | `[10] ↔ [0] ↔ [30]` | 노드 10 | 노드 30 |

첫 붙이기 행에서 `head`와 `tail`이 같은 노드를 가리킵니다. 빈 리스트에 붙이면 새 노드가 첫 노드이자 마지막 노드이기 때문입니다.
...
`append`에서 `if (this.tail === null)`이 빈 리스트를 가르는 줄입니다. 이 분기가 없으면 첫 붙이기에서 `head`가 `null`로 남아 앞에서부터 읽기가 아무것도 돌려주지 않습니다. 원소가 몇 개이든 한 번의 붙이기가 고치는 참조는 셋을 넘지 않습니다.

#### 3-1. XOR 연결 리스트 — 이웃 id 둘을 XOR 한 값 하나만 둔다

C나 Rust처럼 주소를 정수로 다룰 수 있는 언어에서는 노드 주소 두 개를 XOR 해 저장합니다. TypeScript에는 주소를 정수로 꺼내는 방법이 없으므로, 이 구현은 노드마다 1부터 차례로 번호 `id`를 붙이고 `Map`에 `id → 노드`를 기록합니다. 노드는 `{ id, value, xorId }`를 저장하고, `xorId`는 이전 노드 id와 다음 노드 id를 XOR 한 값입니다. 리스트는 첫 노드 id `headId`, 마지막 노드 id `tailId`, 다음에 줄 번호 `nextId`를 기억합니다. 이웃이 없는 자리는 `NIL = 0`입니다.

```text
nodes (Map)   1 → { id: 1, value: 10, xorId: 2 }
              2 → { id: 2, value: 0,  xorId: 2 }
              3 → { id: 3, value: 30, xorId: 2 }
headId = 1, tailId = 3, nextId = 4
```
...
2행과 5행은 같은 id 2 노드의 같은 값 2를 읽었습니다. 앞에서 온 순회는 `prev`가 1이라 3을 얻었고, 뒤에서 온 순회는 `prev`가 3이라 1을 얻었습니다. 앞에서부터 i번째 노드의 id를 `id(i)`, 양 끝 바깥을 `id(-1) = id(n) = 0`으로 두면 저장값은 `x(i) = id(i - 1) ^ id(i + 1)`입니다. 어느 방향이든 직전 노드 id를 XOR 하면 같은 수가 두 번 XOR 되어 사라지고 반대쪽 이웃만 남습니다.
...
붙이기 규칙도 같은 식에서 나옵니다. 마지막 노드 `id(n - 1)`의 저장값은 `id(n - 2) ^ 0`이고, 새 노드가 붙으면 `id(n - 2) ^ id(n)`이어야 합니다. 차이는 0이 새 id로 바뀐 것뿐이므로 새 id를 한 번 XOR 하면 됩니다.

#### 짚고 가기 — XOR 연결 리스트에서 id를 0부터 주거나 prev에 다음 id를 넣으면
...
**id를 0부터 주면 첫 노드의 번호가 `NIL`과 같아집니다.** 원소 하나를 붙인 직후에 `headId`와 `tailId`가 0이라 리스트가 빈 것처럼 보이고, 둘째 붙이기도 빈 리스트에 붙이는 경우로 처리되어 `headId`를 덮어씁니다. 그래서 처음 붙인 10을 어느 순회도 읽지 못합니다. 두 순회가 서로 뒤집은 결과를 내므로, 두 방향만 비교하는 검사는 이 실수를 놓치고 붙인 값과 대조하는 검사가 잡습니다.
...
#### 3-2. XOR 연결 리스트 — 빈 수열부터 두 방향 읽기까지 실행하기

아래 실행은 빈 수열을 읽고 10, 0, 30을 붙인 뒤 두 방향으로 읽습니다. 순회는 노드 하나를 읽을 때마다 한 걸음으로 셉니다. 노드 표의 칸은 `값/xorId`입니다. `10/x2`는 값이 10이고 `xorId`가 2인 노드입니다. T3에서 붙인 값 0은 노드의 값일 뿐이고, 읽기는 값이 아니라 id를 `NIL`과 비교하므로 T6과 T9에서 멈추지 않습니다. 마지막 걸음 T10은 **`[30, 0, 10]`을 반환합니다.**
<!--result:walk=[30,0,10]-->
...
```ascii
T1 빈 수열 읽기 → []
T2~T4 append(10) · append(0) · append(30) → 노드 표 10/x2 0/x2 30/x2, headId 1, tailId 3
T5~T7 toArray()의 세 걸음 → [10] → [10 0] → [10 0 30]
T8~T10 toArrayReverse()의 세 걸음 → [30] → [30 0] → [30 0 10]
```

걸음마다 노드 표와 순회 상태입니다. 참조 구현과 같은 절차의 사본으로 실행했고, 호출마다 반환값과 비용이 참조 구현과 같은지 확인했습니다.

<!--proof:walk-viz-->

```text
단계   노드 표 (id 1 · 2 · 3)   headId   tailId   prev   curr     담은 값
T1     · · ·                         0        0      —      —          []
T2     10/x0 · ·                     1        1      —      —           —
T3     10/x2 0/x1 ·                  1        2      —      —           —
T4     10/x2 0/x2 30/x2              1        3      —      —           —
T5     10/x2 0/x2 30/x2              1        3      0      1        [10]
T6     10/x2 0/x2 30/x2              1        3      1      2      [10 0]
T7     10/x2 0/x2 30/x2              1        3      2      3   [10 0 30]
T8     10/x2 0/x2 30/x2              1        3      0      3        [30]
T9     10/x2 0/x2 30/x2              1        3      3      2      [30 0]
T10    10/x2 0/x2 30/x2              1        3      2      1   [30 0 10]
```

같은 걸음에서 확인한 조건과 계산, 참조 구현이 센 비용입니다. 비용은 노드를 `Map`에서 찾을 때 1, 새 노드를 만들 때 1입니다.
...
`#walk` 하나가 두 방향 읽기를 모두 처리하고 출발 id만 다릅니다. `#at`은 표에 없는 id를 따라가면 오류를 던지므로, 잘못 계산한 이웃 id가 조용히 넘어가지 않습니다.
...
이 코드는 [참조 구현](./_reference/xorLinkedList.ts)에서 계측 코드만 제외한 것입니다. XOR 연결 리스트를 TypeScript로 구현하는 한 가지 방법이며, 앞의 두 구현도 같은 연산 의미와 비용 요구를 만족합니다.

#### 4-1. Rust XOR 연결 리스트 — 노드 주소를 직접 XOR 한다

TypeScript XOR 구현은 주소 대신 번호를 쓰고, 번호로 노드를 찾으려고 `Map`을 둡니다. 원소 하나마다 노드 필드 셋과 `Map` 항목 둘이 필요하므로 이중 연결 리스트보다 메모리를 더 씁니다. 이 구조가 아끼려던 메모리는 번호가 아니라 **메모리 주소를 XOR** 해야 생깁니다. Rust 표준 라이브러리에는 포인터의 주소를 정수로 꺼내고, 계산한 정수를 다시 포인터로 바꾸는 함수가 있으므로 같은 구조를 주소로 구현할 수 있습니다.

Rust 노드는 값 `value`와, 이전 노드 주소와 다음 노드 주소를 XOR 한 정수 `xor_addr` 두 필드만 가집니다. 리스트는 첫 노드 포인터 `head`와 마지막 노드 포인터 `tail`을 기억합니다. 번호로 노드를 찾는 표도, 다음 번호를 세는 카운터도 없습니다. TypeScript 구현의 무엇이 무엇으로 바뀌는지 짝지으면 이렇습니다.
...
다음은 `rust/structures/src/xor_linked_list.rs`의 구현입니다. 계약의 세 연산을 `append`, `to_array`, `to_array_reverse`로 옮겼고 값은 정수 `i64`입니다. `count` 필드와 `size()`는 계약의 연산이 아니라 `cargo test`가 test vector를 재생하며 확인하는 보조 항목입니다. 그 테스트가 TypeScript 정본과 같은 결과를 내는지 확인합니다.
...
요청한 바이트만 세었고, 할당기가 크기를 올림하거나 관리 정보를 덧붙이는 부분은 세지 않았습니다. TypeScript에서는 XOR 구현이 이중 연결 리스트보다 값 자리를 더 잡았지만, 주소를 직접 XOR 한 Rust 구현에서는 노드가 이중 연결 노드의 3분의 2 크기입니다. 정수로 계산한 주소를 포인터로 쓰는 일에 Rust가 두는 조건은 「TypeScript 의 한계와 대체 언어」에서 다룹니다.

## 파트 2 — 같은 수열, 저장 비용, 선택 기준

### 불변식 — 저장 상태를 한 수열로 읽는 규칙

네 구현이 같은 결과를 내려면, 저장한 상태를 원소의 순서로 해석하는 규칙이 붙이기 뒤에도 유지되어야 합니다. 명세가 요구하는 것은 하나입니다. 어느 시점이든 `toArrayReverse()`는 `toArray()`를 뒤집은 것과 같아야 합니다. 두 방향이 서로 다른 수열을 읽으면 그것은 한 수열이 아니라 두 수열입니다. 명세는 특정한 배열 배치나 노드 연결을 요구하지 않고, 각 구현이 자기 저장 규칙으로 이 조건을 지킵니다.

| 구현 | 저장 상태를 읽는 규칙 | 붙이기 뒤 확인할 것 |
| --- | --- | --- |
| 동적 배열 | 0번 칸부터 마지막 칸까지가 수열 | 새 값이 마지막 칸에 있는지 |
| 이중 연결 리스트 | head에서 next를 따라 읽음 | 이웃 a, b에서 `a.next === b`이면 `b.prev === a`인지 |
| XOR 연결 리스트 | headId에서 `xorId ^ prev`를 따라 읽음 | 모든 노드가 `xorId = 이전 id ^ 다음 id`인지 |
| Rust XOR 연결 리스트 | head 포인터에서 `xor_addr ^ prev`를 따라 읽음 | 모든 노드가 `xor_addr = 이전 주소 ^ 다음 주소`인지 |

배열의 붙이기는 끝에 한 칸을 더하고, 두 순회는 같은 배열을 반대 방향으로 읽으므로 규칙이 항상 참입니다. 이중 연결 리스트의 붙이기는 새 노드의 `prev`와 기존 마지막 노드의 `next`를 함께 고칩니다. XOR 연결 리스트의 붙이기는 새 노드와 기존 마지막 노드의 `xorId`를 모두 정의에 맞게 고칩니다. 이 규칙이 유지되면 어느 끝에서 출발해도 매 걸음 실제 이웃으로 이동하고, 반대쪽 끝을 읽은 직후 다음 id가 `NIL`이 되어 멈춥니다.
...
앞에서 읽으면 2번 노드에서 1번으로 되돌아가 같은 노드를 두 번 읽고, 뒤에서 읽으면 2번 노드에서 멈춥니다. 명세의 불변식 검사 함수를 그대로 호출하면 이렇게 보고합니다.
...
현재 원소 수를 n, 빈 수열부터 실행한 붙이기 횟수를 m이라고 하겠습니다. **동적 배열**은 칸이 남아 있으면 붙이기 한 번에 칸 하나를 씁니다. 칸이 가득 차면 두 배 크기로 옮기는데, 용량 8에서 시작한다면 복사하는 원소 수는 `8 + 16 + 32 + … + L`이고 합은 2L보다 작습니다. L개를 복사하려면 그 전에 L번 이상 붙였어야 하므로 m번 붙이는 총비용은 O(m)입니다. 두 순회는 원소 n개를 한 번씩 복사합니다.
...
이 표는 각 구현의 보장입니다. 앞의 계약 표가 붙이기를 상각 O(1)로 허용하는 것은 동적 배열처럼 가끔 복사하는 구현을 배제하지 않기 위해서입니다. 두 연결 리스트는 그보다 강한 최악 O(1)을 지키고, 더 강한 보장을 주는 구현도 계약을 만족합니다. 두 순회는 어떤 구현이든 원소 n개를 돌려주므로 호출 한 번이 n에 비례하고, 여러 호출을 평균 내어 줄일 여지가 없어 최악 O(n)입니다.

이 구현들에는 난수가 없습니다. 상각 O(1)은 흔한 입력의 평균이 아니라 어떤 순서로 m번 호출해도 성립하는 총비용의 상한입니다.

#### 최악을 만드는 입력

이 계약에서는 붙이기의 비용이 값에 의존하지 않고 두 순회에는 인자가 없습니다. 그래서 오류를 드러내는 입력과 비용이 커지는 입력은 구현마다 따로 생각해야 합니다.

| 확인할 입력 | 확인할 동작 |
| --- | --- |
| 동적 배열에서 칸이 막 가득 찬 순간의 붙이기 | 그 한 번이 원소 수만큼 복사하는지, 총비용은 O(m)에 머무는지 |
| 마지막 노드를 기억하지 않는 연결 리스트에 계속 붙이기 | 붙일 때마다 처음부터 이동해 총 `n(n - 1) / 2`번 — 계약의 붙이기 상한을 넘는다 |
| 원소를 많이 붙인 뒤 순회 | 모든 구현에서 원소 수만큼 방문한다 — 계약이 허용한 비용이다 |
| 값 0, 음수, 중복값 붙이기와 원소 한두 개에서 읽기 | 값이나 경계 상태로 끝을 잘못 판단하지 않는지 |
| 원소 셋 이상에서 두 방향 읽기 | XOR 갱신을 덮어쓰기로 바꾼 실수는 원소 셋부터 확인된다 |

세 TypeScript 구현과 참조 구현에 같은 붙이기를 보내고, 단순한 배열로 계산한 결과와 매번 두 순회를 비교할 수 있습니다. 배열은 정답을 확인하기 위한 모델이므로 느려도 괜찮습니다.
...
1,024, 1.999023, —
4,096, 1.999756, 1.000
16,384, 1.999939, 1.000
```

위 수치는 참조 구현에서 붙이기 한 번마다 찾거나 만든 노드 수를 센 것입니다. 원소 수를 네 배로 늘려도 붙이기 한 번의 평균 비용은 2에 머물고 비율은 1입니다. 읽기와 쓰기를 따로 세지 않으며 실제 실행 시간도 아닙니다. 이런 유한한 실행 검사는 잘못된 구현을 찾는 데 도움이 되지만, 모든 크기에 대한 상각 분석을 대신하지는 않습니다.

#### TypeScript 의 한계와 대체 언어

이 저장소는 자료구조마다 TypeScript만으로 계약을 지키고 그 구조의 존재 이유까지 확인할 수 있는지 구분해 둡니다. XOR 연결 리스트의 세 연산은 TypeScript로 표현되고 실제로 지켜집니다. Rust 같은 다른 언어가 있어야 표현할 수 있는 조건은 없습니다. 하지만 이 구조가 만들어진 이유인 **메모리 절약**은 TypeScript에서 측정할 수 없습니다.
...
**주소를 직접 다루는 Rust로 옮기면 절약이 실제로 생깁니다.** 「4-1. Rust XOR 연결 리스트」의 구현은 노드 하나에 16바이트를 요청하고, 같은 테스트의 Rust 이중 연결 노드는 24바이트를 요청합니다. 위 표의 2와 3이 8바이트 워드 단위로 그대로 나온 값입니다.
...
뒤에만 붙이고 전체를 두 방향으로 읽으며 가운데를 건드리지 않는다면 이 계약이 요구에 맞습니다. 어느 구현을 고를지는 별도의 선택입니다. TypeScript에서 이 계약만 필요하면 동적 배열이 가장 단순하고, 붙이기 한 번의 비용이 늘 일정해야 하면 이중 연결 리스트를 검토할 수 있습니다. XOR 연결 리스트는 노드마다 이웃 정보 하나를 줄이는 것이 실제 제약이고, 주소를 정수로 다룰 수 있는 언어로 구현할 때만 그 절약이 생깁니다.

| 먼저 확인할 요구 | 검토할 구현 | 함께 고려할 비용 |
| --- | --- | --- |
| 원소 하나에 값 한 칸만 쓰고 연속된 칸을 차례로 읽기 | 동적 배열 | 칸이 가득 찬 순간의 붙이기가 원소 수만큼 복사 |
| 붙이기 한 번의 비용이 늘 일정하고, 만든 노드를 옮기지 않기 | 이중 연결 리스트 | 원소마다 참조 두 개 |
| 노드당 참조 하나를 줄이기, 주소를 정수로 다루는 언어 | XOR 연결 리스트 | 이웃 하나를 알아야 이동 가능, 디버깅이 어려움 |

TypeScript에서는 XOR 연결 리스트가 이중 연결 리스트보다 메모리를 더 쓰므로, 이 저장소의 TypeScript 구현은 원리와 그 대가를 확인하는 용도입니다. 노드가 실제로 작아지는 것은 Rust 구현에서 16바이트 대 24바이트로 확인했습니다.

#### 이 구조를 떠올리게 하는 연산 조합

한 방향으로만 읽으면 단일 연결 리스트로 충분합니다. 붙이기가 없고 읽기만 있으면 고정 배열로 충분합니다. 이 계약은 붙이기가 계속 이어지는 동안 두 방향 읽기가 함께 필요할 때 맞습니다.

```text
단일 연결 리스트:   뒤에 붙이고 앞에서부터만 읽기
고정 배열:          한 번 채운 뒤 두 방향으로 읽기
이 계약:            계속 붙이면서 앞에서부터와 뒤에서부터 읽기
```

측정값을 계속 기록하면서 화면에는 최신순으로, 파일에는 발생순으로 내보내는 요구를 연산으로 옮기면 `append`, `toArrayReverse`, `toArray`입니다. 측정 m번이면 붙이기 m번, 화면을 갱신할 때마다 순회 한 번, 파일로 내보낼 때마다 순회 한 번입니다. 세 연산이 모두 계약에 있습니다. 반면 되돌리기와 다시 하기 기록은 되돌린 뒤 새 작업을 하면 뒤쪽 기록을 버려야 하는데, 그 연산이 이 계약에 없습니다. 측정 기록 예는 동적 배열로도 같은 연산이 되므로, 연산 조합만으로는 XOR 연결 리스트를 고를 이유가 생기지 않습니다. **노드당 메모리가 제약이며 주소를 정수로 다루는 언어로 구현한다**는 조건이 함께 있어야 합니다.
...
큰 복사를 피하려는 조건에서는 연결 리스트가 유리하고, 원소당 저장을 줄이려는 조건에서는 배열이 유리합니다. 원소당 저장을 줄이면서 노드로 잇고 싶다면 XOR 연결 리스트가 후보이고, 그 이득은 언어가 주소를 정수로 내줄 때만 생깁니다. 이 계수만으로 어느 쪽이 실제로 더 빠르다고 결론 내릴 수는 없습니다.

### 스스로 점검하기

1. T3에서 10에 이어 0을 붙였습니다. 이때 `xorId`가 바뀐 노드는 몇 개이고 각각 어떤 값이 되었을까요? **답:** 두 개입니다. 새 노드 id 2의 `xorId`는 이전 노드 id인 1이고, 기존 마지막 노드 id 1의 `xorId`는 `0 ^ 2 = 2`가 됩니다. 붙인 값이 0이 아니라 20이어도 id는 같으므로 달라지는 것이 없습니다.
2. T8\~T10은 T5\~T7과 같은 노드 표를 읽었지만 결과가 반대입니다. 왜 같은 `xorId` 값에서 반대 방향의 노드가 계산될까요? **답:** `xorId`는 두 이웃의 XOR이고, 순회가 들고 있는 `prev`는 어느 쪽에서 왔는지에 따라 다릅니다. 앞에서 오면 이전 이웃이 지워져 다음 이웃이 남고, 뒤에서 오면 다음 이웃이 지워져 이전 이웃이 남습니다.
3. `toArrayReverse()`를 `this.toArray().reverse()`로 바꾸면 결과는 같습니다. 그 대신 기존 마지막 노드의 값을 덮어쓰는 실수가 있을 때, 명세의 불변식은 그 실수를 여전히 발견할까요? **답:** 발견하지 못합니다. 두 방향이 같은 코드에서 나오므로 뒤집은 결과가 항상 일치합니다. 그 자리는 붙인 값의 배열과 순회 결과를 대조하는 검사가 대신 잡습니다. 앞에서 읽은 결과가 `[10 20 20 10]`이 되기 때문입니다.
4. T2에서 TypeScript 구현은 첫 노드에 id 1을 주었습니다. Rust 구현에는 id를 1부터 줘야 한다는 주의가 없는데, 무엇이 끝 표시 0과 겹칠 가능성을 없앴을까요? **답:** `Box`가 할당한 주소는 널 포인터가 아니므로 0이 될 수 없습니다. 반면 `prev`에 다음 주소를 넣는 실수는 TypeScript처럼 오류로 멈추지 않고, 이웃이 아닌 주소를 읽는 정의되지 않은 동작이 됩니다.
