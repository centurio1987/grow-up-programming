...
> = {
  ...
  // 헤더 불변식 절의 넷. 넷 다 관측 경로가 둘이라 구현이 따로 유지하면 갈린다.
  invariants: [
    {
  ...
  scenarios: [
    ...
    },
    {
      ...
      //
      // 조회가 트리를 고쳐 쓰는 계열은 찾은 자리를 뿌리로 끌어올린다. 오름차순으로
      // 훑으면 다음 조회가 늘 반대쪽 끝에 있으므로 **단일 호출 하나가 남은 사슬
      // 길이만큼** 든다. 같은 구현이 무작위 조회에서는 로그 근처에 머문다 —
      // 그래서 무작위만 두면 `amortized` 구현이 `worst` 계약을 통과한다.
      run: (impl, n, ctx) => {
