/**
 * `tree/avlTree` 계약 스위트(규약2).
 *
 * **여기에는 스위트가 없다.** `tree/redBlackTree` 의 `ContractSpec` 을 그대로 내보내고
 * 이름표만 바꾼다. 계약이 같으므로(`./avlTree.ts` 헤더 맨 앞) 스위트도 같아야 하고,
 ...
 * 표면으로 맞췄기 때문이다(`height()` 를 빼고 `range` 를 넣은 자리).
 */
...
export type AVLTreeContract<T> = RedBlackTreeContract<T>;

export const avlTreeContract: typeof redBlackTreeContract = {
  ...redBlackTreeContract,
  name: "AVLTree",
};
