 ...
 * 위치 기반 접근(k 번째 원소·순위 조회)은 이 계약에 없다. 양 끝(최소·최대)을 읽는 일과
 * 중복을 포함한 총 원소 수를 읽는 일도 없다.
 *
 ...
 * 있다. **순서 질의 36 가운데 제거가 난 자리가 이 계약과 `tree/orderStatisticTree` 둘뿐이고**,
 * 그 절의 「갈라진 계약」 소절이 까닭을 적는다. 판정의 결론을 목적 문단 안에 적으면 다음
 * 판정이 자기 메아리를 근거로 읽는다(§원칙 A A5′-2).
 *
 * **불변식.** 하나다. **관측 경로가 둘**이라서 성립한다 — 구현이 두 경로를 따로
 * 유지할 수 있고, 따로 유지하는 순간 갈린다. 어떤 연산 뒤에도 성립해야 한다.
 * 1. 임의의 `x`에 대해 `count(x)`는 `toArray()` 안에서 `x`와 동등한(비교자가 0) 원소의
 *    개수와 같다. 다중도를 읽는 길이 둘이다.
 *
 * 판별 절차(§규약1 「불변식 판별 절차」)를 **세 행을 뺀 표면에 다시 돌린 결과다.** 셋 가운데
 * 둘이 사라졌다. 「`toArray().length === size()`」는 원소 수를 읽는 두 경로 가운데 세어 둔
 * 쪽이 표면에서 사라져 경로가 `toArray()` 하나로 줄었고, 「`min()`·`max()` 가 `toArray()` 의
 * 양 끝과 동등하다」는 양 끝을 읽는 두 경로 가운데 끝점 행 쪽이 통째로 사라져 역시 경로가
 * `toArray()` 하나로 줄었다. 둘 다 첫 번째 물음에서 걸린다. 늘어놓은 배열이 비내림차순이고
 * 담긴 원소 전부인지는 `toArray` 행의 의미라 축1이 참조 모델과 대조한다.
 *
 ...
 * - 없는 원소의 `delete`는 예외가 아니라 `false`다. 예외를 던지는 연산이 없다.
 *
 * **검증 등급.** `complexity`.
