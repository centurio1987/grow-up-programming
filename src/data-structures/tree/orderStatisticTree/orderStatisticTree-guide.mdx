...
값 `{1, 3, 4, 5, 7, 8, 9}`를 담은 균형 BST(각 노드에 서브트리 크기 `size` 저장)에서 `kth(5)`와 `rank(7)`을 수행하는 과정이다. 노드 라벨은 `key (size=…)` 형태다. `status`로 현재 탐색 경로를 표시한다: `active`는 지금 방문 중인 노드, `visited`는 이미 지난 노드다. 정렬 순서는 `1,3,4,5,7,8,9`.

`kth(5)`의 실제 반환값은 `7`(5번째로 작은 값), `rank(7)`의 실제 반환값은 `4`(7보다 작은 원소 `{1,3,4,5}`의 개수)이며, 각 탐색의 마지막 프레임과 일치한다.
...
function withStatus(node, statuses) {
