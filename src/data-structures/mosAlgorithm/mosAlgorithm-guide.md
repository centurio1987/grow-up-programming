...
**핵심 아이디어**: "질의를 영리하게 정렬하면 구간 포인터의 총 이동 거리를 $O((n+q)\sqrt{n})$으로 줄일 수 있다."

여러 구간 질의에 대해 독립적으로 계산하면 $O(qn)$이 걸린다. Mo의 알고리즘은 두 포인터로 현재 구간을 유지하면서 인접한 질의로 조금씩 이동하는 방식을 사용한다. 핵심은 질의를 "$\sqrt{n}$ 크기 블록" 기준으로 정렬하는 것인데, 이렇게 하면 왼쪽 포인터는 블록 내에서 최대 $O(\sqrt{n})$씩, 오른쪽 포인터는 전체에서 단조 이동하여 총 이동량이 $O((n+q)\sqrt{n})$에 수렴한다.

**풀이 구조**
1. 블록 크기 $B = \lfloor\sqrt{n}\rfloor$을 정하고, 질의에 원래 인덱스를 부여한다
2. 질의를 (블록 번호, $r$ 오름차순) 기준으로 정렬한다
3. curL=0, curR=-1로 초기화 후 정렬된 질의를 순회하며 포인터를 이동한다
4. 포인터 이동 시 add/remove 함수로 distinctCount를 $O(1)$에 갱신한다
5. 결과를 원래 질의 인덱스 순서로 복원하여 반환한다

**조건**: 정적 배열에서 구간 질의가 오프라인(미리 주어짐)으로 처리 가능하고, 구간에 원소를 한 칸씩 추가·제거하는 연산이 $O(1)$에 가능한 상황

**대표 예시**: 구간 내 서로 다른 원소의 개수 세기
배열 $[1, 3, 2, 3, 1, 2]$에서 질의 $[1, 4]$, $[0, 2]$, $[2, 5]$ 등 여러 구간의 distinct count를 구할 때, 각 질의를 독립적으로 계산하면 $O(qn)$이지만 Mo의 알고리즘으로 정렬하면 $O((n+q)\sqrt{n})$에 처리할 수 있다.

**언제 쓰나**
오프라인 구간 질의이고 원소 추가/제거가 $O(1)$이며 전처리로 $O(\sqrt{n})$ 개선이 필요할 때 사용한다. 동적 갱신(배열 원소가 바뀜)이 있거나 온라인 질의라면 적합하지 않다.
