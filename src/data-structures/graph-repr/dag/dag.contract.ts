 ...
 * 판정에서 관측되게 하려는 것이다.
 *
 * **축3 시나리오는 `addEdge` 에 둘이다(불변 사실 191 · 211).** 상한의 파라미터가 n 이 아니라 **닿는 부분**
 * k(u, v) 이므로 그 양 끝을 둔다 — 떨어진 쌍을 잇는 쪽(k 가 상수, bound `O(1)`)과 사슬의 끝을 처음에
 * 잇는 쪽(k 가 n, bound `O(n)`)이다. 앞쪽이 넣을 때마다 그래프 전체를 훑는 계열을 겨누고, 뒤쪽이 상한이
 * 실제로 닿는 부분을 따라가는지 본다. k 를 이루는 셋 가운데 「같은 간선을 찾는 차수」만 키우는 쪽(별)은
...
export const dagContract: ContractSpec<DagShell, Model> = {
  ...
  scenarios: [
    ...
    {
      // k 를 상수로 누른다 — 정점 2n 개를 두고 떨어진 쌍만 잇는다. 새 간선이 닿는 부분은 두 정점뿐인데
    ...
    {
      // k 를 n 으로 키운다 — 사슬 0→1→…→n-1 의 끝을 처음에 잇는 간선을 n 번 넣는다. 매번 사이클이라
