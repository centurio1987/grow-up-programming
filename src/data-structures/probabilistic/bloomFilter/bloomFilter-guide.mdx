...
`m = 10`, `k = 2`인 Bloom Filter에 `add("cat")`, `add("dog")`을 수행한 뒤 `has("cat")`와 `has("fish")`를 조회한다(해시 위치는 설명을 위해 고정값 사용). array 패널은 10개 비트 배열이며, 빨간색(`highlight`)은 현재 연산이 검사/설정하는 비트 위치, 회색(`marked`)은 이미 1로 설정된 비트다.

마지막 `has("fish")`의 실제 반환값은 `false` (해시 위치 1번 비트가 0 → 확실히 없음) 이며, 시뮬레이션 마지막 프레임과 일치한다.
