...
`w = 4`, `d = 3`인 Count-Min Sketch에 `update("a",3)`, `update("b",2)`, `update("a",1)`을 수행한 뒤 `estimate("a")`, `estimate("b")`를 조회한다(해시 위치는 설명을 위해 고정). matrix 패널은 d×w 카운터 행렬이며, 강조된 셀(`cells`)은 현재 원소의 d개 해시 위치다. 행1의 col2에서 a와 b가 충돌하지만, estimate는 d개 행의 최솟값을 취해 오차를 줄인다.

마지막 `estimate("b")`의 실제 반환값은 `2` (행0·행2가 2, 충돌난 행1은 6이지만 min으로 보정) 이며, 시뮬레이션 마지막 프레임과 일치한다.
...
export const steps = [
  ...
  {
    title: 'update("a", 3)',
    detail: "a의 해시: h0->1, h1->2, h2->0. 각 위치에 +3.",
    matrix: [
      [0, 3, 0, 0],
      [0, 0, 3, 0],
      [3, 0, 0, 0],
  ...
  {
    title: 'update("b", 2)',
    detail: "b의 해시: h0->2, h1->2, h2->3. 각 위치에 +2. 행1 col2는 a와 충돌.",
    matrix: [
      [0, 3, 2, 0],
      [0, 0, 5, 0],
      [3, 0, 0, 2],
  ...
  {
    title: 'update("a", 1)',
    detail: "a의 위치에 +1. h0->1=4, h1->2=6, h2->0=4.",
    matrix: [
      [0, 4, 2, 0],
      [0, 0, 6, 0],
      [4, 0, 0, 2],
  ...
  {
    title: 'estimate("a") = min(4, 6, 4) = 4',
    detail: "a의 3개 위치 값 [4, 6, 4]의 최솟값 = 4. 실제 빈도와 일치.",
    matrix: [
      [0, 4, 2, 0],
      [0, 0, 6, 0],
      [4, 0, 0, 2],
  ...
  {
    title: 'estimate("b") = min(2, 6, 2) = 2',
    detail: "b의 3개 위치 값 [2, 6, 2]. 행1은 충돌로 6이지만 min으로 2 반환.",
    matrix: [
      [0, 4, 2, 0],
      [0, 0, 6, 0],
      [4, 0, 0, 2],
  ...
  {
    title: '완료: estimate("b") = 2',
    detail: "과소추정 없이 실제 빈도 2를 정확히 반환.",
    matrix: [
      [0, 4, 2, 0],
      [0, 0, 6, 0],
      [4, 0, 0, 2],
...
<AlgorithmSimulation view="matrix" steps={steps} title="Count-Min Sketch: w=4, d=3" />
