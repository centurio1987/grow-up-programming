 ...
 *
 * 여기에는 `runContract` · `trialContract` 호출만 둔다. 무엇을 검사하는지는 `./minHash.contract.ts` 에 있고, 계약 자체는 `./minHash.ts` 헤더
 * 한 곳이다.
 *
 ...
 * `./minHash.contract.ts` 머리말, 반복 실행의 [경험] 수치는 `docs/ORD-006-conventions.md` 의 `S24` 절.
 *
 ...
 * 매개변수(numHashes)로 적힌 수치는 헤더의 판정 문장(닮음 오차 · 실패 확률 · 독립 시행)으로 대체됐다. 결함 fixture 의 자기시험은
...
trialContract(minHashTrials, {
  label: "스텁",
  implementation: {
    module: new URL("./minHash.ts", import.meta.url).href,
    exportName: "MinHash",
  },
});
...
trialContract(minHashTrials, {
  label: "정본",
  implementation: {
    module: new URL("./_reference/minHash.ts", import.meta.url).href,
    exportName: "MinHash",
  },
});
