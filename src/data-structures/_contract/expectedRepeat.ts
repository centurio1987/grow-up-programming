 ...
 * 해상도가 2.5 × 10^−5 라 「0」은 「그보다 작다」까지만 말한다(**[경험]** · B4). 대가는 정본 대상 `expected` 계측이
 * **정확히 두 배**가 되는 것이다 — 1,035 → 2,070(전체 1,812 → 2,847). 자기시험은 선언 없는 합성 시나리오가 섞여
 * 두 배가 아니다 — 4,805 → 9,505(전체 8,674 → 13,374 · `S1` 의 옛 수를 `S2` 절의 정정 문단이 다시 셌다).
 * `worst` · `amortized` 계측은 한 번도 안 는다.
 *
 ...
 * **적대적 시나리오**가 한다(대개 입력이 고정이라 씨앗 축이 하나다 — 다만 `S2` 가 재 보니 적대 27 중 셋은 `ctx.rng` 를 읽는다).
 */
