 ...
 * 2. **옛 정본(`_fixtures/fullGapErasingBuffer.ts`)** — 새 경계 케이스 한 자리와 **무작위 500 회의 76번째 호출**에서
 *    걸리고, 나머지 경계와 축3 은 고친 정본과 한 자리도 다르지 않다. 비용이 값을 읽지 않으므로 축3 이 이 결함을 볼
 *    길이 없다. **무작위가 잡게 된 것은 `KAN-040` `S3` 뒤다** — 원소 수를 읽는 행이 계약에서 빠지면서 같은 seed 의
 *    연산 뽑기가 달라졌고, 그 전에는 500 회 안에서 갈리지 않았다.
...
const REFERENCE_STATS = [
  ...
  // 커서 시나리오는 `KAN-040` `S3` 전에 커서와 원소 수를 한 걸음에 묶어 2 였다. 원소 수를
  // 읽는 행이 빠져 걸음이 커서 하나가 됐고 그만큼 줄었다.
  { ok: true, stats: [1, 1] },
...
describe("gapBuffer — 옛 정본(칸이 꼭 찼을 때 커서를 옮기면 원소를 지운다)", () => {
  test("새 경계 케이스의 크기 8 첫 읽기와 무작위 76번째 호출에서 걸리고, 축3 은 고친 정본과 같다", () => {
    ...
    // `KAN-040` `S3` 뒤로는 무작위도 잡는다(위 머리말 2번).
    expect(randomSplit(erasing)).toBe(
      "75번째 toArray — 관측 [67,97,null,null,null,null,72] / 모델 [67,97,65,14,30,50,72]",
    );
