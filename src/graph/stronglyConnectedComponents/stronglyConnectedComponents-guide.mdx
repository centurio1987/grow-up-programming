...
예시 유향 그래프 `n = 5`, `edges = [[0,1], [1,2], [2,0], [2,3], [3,4], [4,3]]`에 대해 Tarjan 알고리즘을 실행하는 과정이다. 노드 위 표기는 `disc/low`이다. 노란색은 스택에 있는 정점(onStack=frontier), 빨간색은 현재 정점(active), 회색은 확정된 SCC(visited)이다. `low[v] == disc[v]`인 정점이 SCC 루트이며, 그 순간 스택을 비워 SCC를 수집한다.

실제 반환값은 `[[0, 1, 2], [3, 4]]` 이며, 시뮬레이션 마지막 프레임과 일치한다.
