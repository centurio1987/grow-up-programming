...
`n = 4`, `k = 2`, 비용 행렬 `cost[i][j] = (sum a[i..j])^2` (단, `a = [1, 2, 3, 4]`)에 대해 분할 정복 DP를 실행하는 과정이다. 이 비용은 사각 부등식을 만족한다. 표는 `dp[g][i]`(행 `g` = 그룹 수, 열 `i` = 구간 끝)이며 `null`은 미계산 칸, 빨간 셀은 그 단계에서 확정된 칸이다. keyValue 패널은 분할 정복 탐색에서 확정한 최적 분할점 `opt(g, i)`를 보여준다 (단조 증가 확인용).

실제 반환값은 `dp[2][3] = 52` 이며(분할 `[0,2] | [3,3]`), 시뮬레이션 마지막 프레임의 우하단 칸과 일치한다.
