...
예시 방향 그래프 `n = 4`, `edges = [[0,1,4], [0,2,1], [2,1,-2], [1,3,3]]`, `src = 0`에 대해 SPFA를 실행하는 과정이다. 노드 위 숫자는 현재 `dist`(∞는 미도달)이다. 빨간색은 큐에서 막 꺼낸 정점(active), 노란색은 큐에 든 정점(frontier, inQueue=true)이다. 음수 간선 `2→1 (-2)` 때문에 정점 1이 다시 큐에 들어가 재완화되는 점이 핵심이다.

실제 반환값은 `[0, -1, 1, 2]` 이며, 시뮬레이션 마지막 프레임의 거리 라벨과 일치한다.
