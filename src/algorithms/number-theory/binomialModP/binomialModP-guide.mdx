...
고정 입력 `n = 7n`, `k = 3n`, `p = 5n` (즉 `C(7,3) mod 5`)에 대해 `binomialModP`를 실행하는 과정이다. `n >= p` 이므로 Lucas 정리로 자릿수별 분해가 일어난다. `keyValue` 패널은 각 재귀 호출의 인자와 분해 결과를 보여준다. 실제 `C(7,3) = 35 = 5*7 ≡ 0 (mod 5)`.

실제 반환값은 `0n` 이며, 시뮬레이션 마지막 프레임과 일치한다.
