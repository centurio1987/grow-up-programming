...
고정 입력 `a = [3, 2]` (즉 `3 + 2x`), `b = [1, 4]` (즉 `1 + 4x`)에 대해 `fftMultiply`를 실행하는 과정이다. 결과 길이는 `n+m-1 = 3` 이므로 `N = 4`로 0-패딩한다. `keyValue` 패널은 FFT 파이프라인의 각 단계(변환 → 점별 곱 → 역변환 → 반올림)에서의 복소수 배열 상태를 보여준다.

실제 반환값은 `[3, 14, 8]` 이며 (`(3+2x)(1+4x) = 3 + 14x + 8x^2`), 시뮬레이션 마지막 프레임과 일치한다.
