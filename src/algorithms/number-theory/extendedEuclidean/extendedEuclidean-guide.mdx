...
고정 입력 `a = 12n`, `b = 8n` 에 대해 반복문 버전 `extendedEuclidean`을 실행하는 과정이다. `keyValue` 패널은 매 단계의 상태 쌍 `(old_r, r)`, `(old_s, s)`, `(old_t, t)`와 몫 `q`를 보여준다. 불변식 `old_r = old_s*a + old_t*b`, `r = s*a + t*b` 가 항상 유지된다.

실제 반환값은 `{ g: 4n, x: 1n, y: -1n }` 이며 (검증: 12*1 + 8*(-1) = 4), 시뮬레이션 마지막 프레임과 일치한다.
