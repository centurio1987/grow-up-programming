...
고정 입력 `N = 9`, `M = 3`, 데이터 `5 1 8 3 7 2 9 4 6`에 External Merge Sort를 적용한다. Phase 1에서 3개씩 잘라 정렬해 Run 3개를 만들고(R1=[1,5,8], R2=[2,3,7], R3=[4,6,9]), Phase 2에서 각 Run의 선두 원소만 min-heap에 유지하며 K-way merge로 병합한다. priorityQueue 패널은 현재 힙(라벨=값, run 표시), keyValue 패널은 각 Run의 남은 원소와 출력 파일 내용을 보여준다.

실제 반환 결과 파일 내용은 `[1, 2, 3, 4, 5, 6, 7, 8, 9]` 이며, 시뮬레이션 마지막 프레임의 출력과 일치한다.
