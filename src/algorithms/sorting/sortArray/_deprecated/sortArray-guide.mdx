...
**핵심 아이디어**: "배열을 절반씩 재귀적으로 나눠 정렬하고, 두 정렬된 절반을 선형 시간에 병합한다."

비교를 반복하면서 정렬하는 모든 알고리즘은 $O(N \log N)$이 이론적 하한이다. Merge Sort는 이 하한을 최악 케이스에서도 보장하는 대표적인 알고리즘이다. 배열을 절반씩 나누면 재귀 깊이가 $\log N$이 되고, 각 레벨에서 병합 비용의 합이 $O(N)$이므로 전체가 $O(N \log N)$이다.

**풀이 구조**
1. 배열 복사. `mergeSort(B, 0, N-1)` 호출
2. `lo >= hi`이면 반환 (기저 케이스)
3. `mid = lo + floor((hi - lo) / 2)` 계산
4. `mergeSort(A, lo, mid)`와 `mergeSort(A, mid+1, hi)` 재귀 호출
5. 두 정렬된 절반을 두 포인터로 병합(`left[i] ≤ right[j]`이면 왼쪽 우선 — 안정 정렬)
6. 정렬 완료된 배열 반환

**조건**: 어떤 입력에도 $O(N \log N)$ 보장. 추가 $O(N)$ 공간 필요. 안정 정렬이므로 같은 값의 원소는 원래 순서 유지.

**대표 예시**: 임의 정수 배열의 범용 정렬
값 범위가 넓거나($[-10^9, 10^9]$) 음수가 포함되어 비교 기반 정렬이 필요할 때, 최악 케이스가 존재하는 Quicksort보다 Merge Sort가 안전하다. 링크드 리스트 정렬에서도 포인터 조작으로 $O(1)$ 추가 공간에 구현 가능하다.

**언제 쓰나**
최악 케이스 $O(N \log N)$을 보장해야 하거나, 안정 정렬이 필요할 때 선택한다. Quicksort의 최악 케이스($O(N^2)$)가 걱정되거나, 데이터가 이미 부분 정렬되어 있지 않은 일반적인 경우에 적합하다.
...
고정 입력 `A = [3, 1, 3, 2]`에 대해 Merge Sort를 실행하는 과정이다. 배열은 분할되어 길이 1까지 내려간 뒤, 두 정렬된 절반이 두 포인터로 병합된다. 빨간색(`highlight`)은 현재 비교 중인 원소, 회색(`marked`)은 이미 병합되어 정렬이 확정된 구간, 포인터 `i`/`j`는 좌우 구간의 현재 위치를 가리킨다.

실제 반환값은 `[1, 2, 3, 3]` 이며, 시뮬레이션 마지막 프레임의 배열과 일치한다.
