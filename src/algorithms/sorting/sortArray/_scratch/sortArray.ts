...
function sortArrayOpt(A: number[]): number[] {
  const R = A.slice();
  msRange(R, new Array(A.length), 0, A.length - 1);
  return R;
}
...
console.log(
  "sim [3,1,3,2] →",
  JSON.stringify(sortArrayBasic([3, 1, 3, 2])),
  "(기대 [1,2,3,3])",
);
// problem 예시 + 엣지
const cases = [[5, 2, 8, 1], [1], [3, 3, 3], [-5, 10, -9, 0, 10], [2, 1]];
for (const c of cases)
  console.log(JSON.stringify(c), "→", JSON.stringify(sortArrayBasic(c)));
// D6: < 로 바꾸면 안정성 깨짐(동일 키 객체 순서) — 원시수는 값 동일해 티 안 나므로, 원본 불변 위반 함정 검증
const orig = [3, 1, 2];
const res = sortArrayBasic(orig);
console.log(
  "원본 불변?",
  JSON.stringify(orig) === "[3,1,2]",
  "결과 새배열?",
  res !== orig,
);
// 무작위 200회 3-way 교차검증
...
console.log(
  `무작위 교차검증 200회: ${ok}/200 일치 (naive==basic==opt==Array.sort)`,
);
