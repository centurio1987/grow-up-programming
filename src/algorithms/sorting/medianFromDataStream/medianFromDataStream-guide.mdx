...
수 `1, 2, 3, 4, 5`를 순서대로 `addNum`으로 삽입하며 두 힙의 상태와 중앙값 변화를 추적한다. `lo`는 하위 절반을 담는 max-heap(top=최댓값), `hi`는 상위 절반을 담는 min-heap(top=최솟값)이다. priorityQueue 패널은 `hi`(min-heap) 내부 상태를, keyValue 패널은 `lo` 내용과 현재 `findMedian()` 값을 보여준다. 크기 불변식 `|lo|-|hi| in {0,1}`과 값 불변식 `lo.top() <= hi.top()`이 매 삽입 후 유지된다.

마지막 `findMedian()` 반환값은 `3` (N=5, 홀수 → lo.top()) 이며, 시뮬레이션 마지막 프레임과 일치한다.
