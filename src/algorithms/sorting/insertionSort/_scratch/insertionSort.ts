...
for (const c of cases) {
  ...
  assertEqual(
    insertionSortOptimized([...c]),
    expected,
    `optimized ${JSON.stringify(c)}`,
  );
  assertEqual(
    insertionSortNaive([...c]),
    expected,
    `naive ${JSON.stringify(c)}`,
  );
}
...
// original-array immutability check
{
  ...
  console.log(
    "immutability base:",
    JSON.stringify(A) === JSON.stringify(copyBefore)
      ? "OK (안 변함)"
      : "FAIL (변함)",
  );
  insertionSortOptimized(A);
  console.log(
    "immutability optimized:",
    JSON.stringify(A) === JSON.stringify(copyBefore)
      ? "OK (안 변함)"
      : "FAIL (변함)",
  );
...
// pedagogical trace: A = [3,1,4,1,2] with base algorithm, print intermediate B after each outer i
{
  function traced(A: number[]) {
    ...
    for (let i = 1; i < B.length; i++) {
      ...
      B[j + 1] = key;
      console.log(
        `i=${i}: key=${key} -> B=${JSON.stringify(B)} (이동 ${moves}회)`,
      );
    }
    return B;
  }
...
console.log(
  "bugSwapDirection([3,1,4,1,2]) =",
  JSON.stringify(bugSwapDirection([3, 1, 4, 1, 2])),
);
{
  const A = [3, 1, 2];
  const before = [...A];
...
// binary search comparisons count vs linear for base vs optimized on nearly sorted large-ish input
{
  ...
  console.log(
    "N=1000 sorted: linear compares=",
    countCompares(sortedArr, "linear"),
    "binary compares=",
    countCompares(sortedArr, "binary"),
  );
  console.log(
    "N=1000 reversed: linear compares=",
    countCompares(reversedArr, "linear"),
    "binary compares=",
    countCompares(reversedArr, "binary"),
  );
}
...
console.log(
  "bugHiOffByOne([3,1,4,1,2]) =",
  JSON.stringify(bugHiOffByOne([3, 1, 4, 1, 2])),
);
