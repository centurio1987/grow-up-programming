...
고정 입력 `s = "banana"`에 대해 접미사 배열을 배가(doubling) 방식으로 구하는 과정이다. array 패널은 원본 문자열의 문자들이며, keyValue 패널은 각 라운드의 정규화된 순위 배열 `rank`(원본 인덱스 순서)와 현재 정렬 결과 `sa`를 보여준다. gap을 1, 2로 키우며 두 순위 쌍 `(rank[i], rank[i+gap])`으로 재정렬한다.

실제 반환값은 `[5, 3, 1, 0, 4, 2]` 이며, 시뮬레이션 마지막 프레임의 `sa`와 일치한다.
...
export const steps = [
  ...
  {
    title: "gap=1 키 구성",
    detail: "key(i)=(rank[i], rank[i+1] or -1): i0(1,0) i1(0,2) i2(2,0) i3(0,2) i4(2,-1) i5(0,-1).",
    array: chars,
    entries: [
      { label: "gap", value: 1 },
      { label: "keys", value: "(1,0)(0,2)(2,0)(0,2)(2,-1)(0,-1)" },
      { label: "sa", value: "[0,1,2,3,4,5]" },
    ],
  },
  {
    title: "gap=1 정렬 + 순위 재부여",
    detail: "키 기준 정렬 → sa=[5,1,3,0,4,2]. 같은 키는 같은 순위 → rank=[2,1,4,1,3,0].",
    array: chars,
    entries: [
      { label: "gap", value: 1 },
      { label: "rank (i순)", value: "[2,1,4,1,3,0]" },
      { label: "sa", value: "[5,1,3,0,4,2]" },
    ],
  },
  {
    title: "gap=2 키 구성",
    detail: "key(i)=(rank[i], rank[i+2] or -1): i0(2,4) i1(1,1) i2(4,3) i3(1,0) i4(3,-1) i5(0,-1).",
    array: chars,
    entries: [
      { label: "gap", value: 2 },
      { label: "keys", value: "(2,4)(1,1)(4,3)(1,0)(3,-1)(0,-1)" },
      { label: "sa", value: "[5,1,3,0,4,2]" },
    ],
  },
  {
