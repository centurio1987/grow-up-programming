...
입력 `A = [1, 3, 2, 7]` (N=4)로 세그먼트 트리를 빌드한 뒤 `ops = [query(1,3), update(i=2,v=0), query(1,3)]`을 처리한다. `array` 패널은 1-indexed 트리 배열(노드 1=루트 `[0,3]`, 2=`[0,1]`, 3=`[2,3]`, 4..7=리프 `[0,0]`,`[1,1]`,`[2,2]`,`[3,3]`; 인덱스 0 미사용), `highlight`(빨강)는 현재 방문/재계산 중인 노드, `keyValue` 패널은 논리 배열 `A`와 진행 상태다.

실제 반환값은 `[2, 0]` 이며(첫 query는 `min(3,2,7)=2`, update로 `A[2]`가 2→0이 된 뒤 두 번째 query는 `min(3,0,7)=0`), 시뮬레이션 마지막 프레임과 일치한다.
