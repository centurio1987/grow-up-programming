...
);
{
  ...
  const correct = segmentTreeRangeMinRecursive(A, [
    { type: "query", l: 0, r: 0 },
  ])[0];
  ...
  try {
    ...
    console.log(
      `버그(AND로 잘못 쓴 경우) → ${(err as Error).constructor.name}: 재귀가 멈추지 않아 스택 오버플로우로 죽는다`,
    );
  }
}
...
console.log("\n=== 함정 재현: update에서 상향 재계산 줄을 빠뜨리면? ===");
{
  ...
  console.log(
    `update(i=2,v=0) 후 query(1,3): 정답=0, 재계산 누락 버그 결과=${buggyResult} (리프는 바뀌었지만 부모가 안 바뀌어 옛 값 3을 그대로 봄)`,
  );
}
...
console.log("\n=== 함정 재현: 반복 버전에서 lo++/--hi 순서를 서로 바꾸면? ===");
{
  ...
  function buggyQuery(l: number, rExclusive: number): number {
    ...
    while (lo < hi) {
      if (lo & 1) {
        res = Math.min(res, tree[lo]!);
      } // lo++ 누락 → 같은 노드를 무한 재방문 위험
      if (hi & 1) {
        res = Math.min(res, tree[hi]!);
        hi--;
      } // 순서 반대: 감소 전에 읽음(범위 밖 원소 포함 가능)
      lo >>= 1;
      hi >>= 1;
    }
    return res;
  ...
  try {
    ...
    buggy = res;
    console.log(
      `query(1,4): 정답=${correct}, lo++/--hi 순서를 깬 버그 결과=${buggy} (iter=${iter}, 조기 종료 없이는 lo가 절대 안 늘어나 무한루프)`,
    );
  } catch (err) {
    console.log(
      `query(1,4): 정답=${correct}, 버그는 예외 발생 - ${(err as Error).message}`,
    );
  }
}
...
);
{
  ...
  for (let trial = 0; trial < 200; trial++) {
    ...
    if (sN !== JSON.stringify(rR) || sN !== JSON.stringify(rI)) {
      allOk = false;
      console.error(
        `MISMATCH trial=${trial} N=${N} A=${JSON.stringify(A)} ops=${JSON.stringify(ops)}`,
      );
      console.error(
        `  naive=${sN} recursive=${JSON.stringify(rR)} iterative=${JSON.stringify(rI)}`,
      );
    }
  }
  console.log(
    allOk ? "OK   무작위 200회 전부 일치" : "FAIL 무작위 교차검증 불일치 발견",
  );
