...
**핵심 아이디어**: "min의 멱등성(min(x,x)=x) 덕분에 두 구간이 겹쳐도 결과가 변하지 않으므로, 임의 구간을 두 개의 2^k-구간으로 덮어 O(1)에 최솟값을 구할 수 있다."

Sparse Table은 각 인덱스에서 시작하는 2^k 길이 구간의 최솟값을 O(N log N)에 미리 계산해두는 자료구조다. 질의 시 구간 길이에서 가장 큰 2^k를 구하면, 왼쪽 끝에서 시작하는 구간과 오른쪽 끝에서 끝나는 구간(두 구간은 겹칠 수 있음)의 최솟값 중 작은 것이 답이다. 갱신이 없는 정적 배열에서 O(1) 질의를 달성하는 유일한 방법이다.

**풀이 구조**
1. st[0][i] = A[i]로 기저 레벨을 초기화한다.
2. k=1..log2(N)에 대해 `st[k][i] = min(st[k-1][i], st[k-1][i+2^(k-1)])`로 테이블을 채운다.
3. 각 질의 (l, r)에서 `k = floor(log2(r-l+1))`을 계산한다.
4. `min(st[k][l], st[k][r - 2^k + 1])`을 반환한다.

**조건**: 배열이 정적(갱신 없음)이어야 한다. min, max, gcd처럼 멱등성을 만족하는 연산에만 적용 가능. 합 연산에는 사용 불가.

**대표 예시**: `A=[2,4,3,1,6,7,8,9], query=(0,4)`
st[2][0]=min(A[0..3])=1, st[2][1]=min(A[1..4])=1. k=floor(log2(5))=2. min(st[2][0], st[2][1])=1. 정확히 min(2,4,3,1,6)=1.

**언제 쓰나**
정적 배열에서 구간 최솟값(또는 최댓값, gcd 등)을 수만 번 이상 반복 조회해야 하는 문제에서 사용한다. 세그먼트 트리보다 질의가 빠르고(O(1) vs O(log N)) 구현도 단순하지만, 갱신이 불가하다는 점이 한계다.
...
입력 `A = [2, 4, 3, 1, 6, 7]`로 Sparse Table을 전처리한 뒤 `queries = [(0,4), (2,5)]`를 처리하는 과정이다. `array` 패널은 원본 배열 `A`이고, `highlight`(빨강)는 현재 질의/계산이 덮는 두 개의 `2^k` 구간 인덱스, `marked`(회색)는 질의 구간 `[l, r]`이다. `keyValue` 패널은 전처리된 테이블 레벨(`st[k]`)과 진행 상태를 보여준다.

실제 반환값은 `[1, 1]` 이며(`min(A[0..4])=1`, `min(A[2..5])=1`), 시뮬레이션 마지막 프레임과 일치한다.
