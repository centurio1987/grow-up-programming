...
입력 `arr = [1, 3, 2, 3, 1, 2]`, `queries = [[1,4], [0,2], [2,5]]`에 대해 Mo의 알고리즘으로 구간별 distinct count를 구하는 과정이다. 블록 크기 `B = floor(sqrt(6)) = 2`. 질의는 `(블록번호, r)` 기준으로 정렬되어 `[0,2](orig 1) → [1,4](orig 0) → [2,5](orig 2)` 순서로 처리된다. array 패널의 `pointers`는 현재 구간 `curL/curR`, `marked`(회색)는 활성 구간 `[curL, curR]`이다. keyValue 패널은 `distinctCount`와 각 질의의 답이다.

실제 반환값은 (원래 질의 순서로) `[3, 3, 3]` 이며, 시뮬레이션 마지막 프레임의 answers와 일치한다.
