...
$\text{freq}[v]$는 현재 구간 안에서 값 $v$가 등장한 횟수를 정확히 세는 함수이고, $\text{distinctCount}$는 그중 빈도가 양수인 값의 개수(즉 구간에 실제로 존재하는 서로 다른 값의 수)입니다. 이 두 식이 루프의 모든 순간에 성립한다는 것이 위 산문 불변식의 정식 표현이에요.
...
- **이동 순서: 확장을 먼저, 축소를 나중에.** 진짜 필요조건은 "`remove`는 현재 활성 구간 안에 실제로 들어 있는 원소에만 호출돼야 한다"는 것이고, 확장 먼저·축소 나중은 이를 어기지 않는 안전한 순서 중 하나예요. 이 조건을 깨는 축소 순서를 쓰면 중간에 `curR`이 `curL`보다 작아지는 순간이 생겨, `remove`가 구간에 없는 원소를 빼는(빈도가 음수가 되는) 사고가 납니다.
...
이를 **홀짝 정렬 트릭(odd-even trick)**이라고 부릅니다. 점근 복잡도는 그대로 `O((n+q)√n)`이지만, 블록 경계에서 벌어지던 큰 왕복을 줄여 `curR` 이동의 상수항을 낮추는 **휴리스틱**이에요. 실제 효과의 크기는 질의 분포와 구현에 따라 달라지며, 항상 극적으로 빨라진다고 보장할 수는 없습니다.
...
홀짝 정렬까지 적용한 지금, 앞서 평방 분할 절에서 세운 목표 $O((n+q)\sqrt{n})$을 그대로 달성했습니다 — $n=q=10^4$ 기준 애초 목표였던 약 200만 회($O((n+q)\sqrt{n}) \approx 2\times10^6$) 규모의 add/remove 포인터 이동 상한 안에 들어오고(정렬 비용 $O(q\log q)$는 이 항에 지배되어 별도), 홀짝 트릭은 이 점근 상한을 유지한 채 실측 상수만 줄인 것입니다.
