...
**핵심 아이디어**: "거래 횟수 t와 날짜 i를 분리해, 각 거래 번호마다 최적 매수·매도 상태를 롤링으로 갱신한다."

k회 거래 제한이 있는 주식 문제는 거래 횟수를 외부 루프, 날짜를 내부 루프로 두는 DP로 해결한다. 각 거래 번호 t에 대해 "t번째 매수를 완료한 상태의 최대 누적 이익"과 "t번째 매도를 완료한 상태의 최대 누적 이익"만 추적하면 된다. 이 두 값이 이전 거래의 최적 결과를 자동으로 이어받는다.

**풀이 구조**
1. `buy[0..k] = -INF`, `sell[0..k] = 0`으로 초기화한다.
2. 외부 루프 t = 1..k, 내부 루프 i = 0..N-1을 순회한다.
3. `buy[t] = max(buy[t], sell[t-1] - prices[i])`: i일에 t번째 매수.
4. `sell[t] = max(sell[t], buy[t] + prices[i])`: i일에 t번째 매도.
5. `max(sell)`을 반환한다.

**조건**: 동시에 두 주식을 보유할 수 없음. 매수 후 매도 순서 보장. k ≥ ⌊N/2⌋이면 무제한 거래와 동일.

**대표 예시**: `k=2, prices=[3,2,6,5,0,3]`
t=1 루프에서 `sell[1]=4`(2일 매수, 6일 매도), t=2 루프에서 `sell[1]=4`를 기반으로 0일 재매수 후 3일 매도해 `sell[2]=7`을 달성한다.

**언제 쓰나**
거래 횟수 제한이 있는 최적화 문제에서 사용한다. k=1이면 단순 최소-최대 추적, k=무제한이면 상승분 합산이 더 빠르지만, 일반 k에는 이 O(Nk) DP가 표준 접근이다.
...
입력 `k = 2`, `prices = [3, 2, 6, 5, 0, 3]`에 대해 거래 횟수 상태 DP를 실행하는 과정이다. 날짜 `i`를 바깥, 거래 번호 `t`를 안쪽 루프로 돌려 같은 날 안에서 `t=1 → t=2` 순으로 갱신해야 시간 순서가 보장된다(같은 날 `sell[t-1]`은 그 날까지의 매도만 반영). `array` 패널에서 `highlight`(빨강)는 처리 중인 날 `i`이고, `keyValue` 패널은 `buy[1..2]`(t번 매수 완료 최대 이익)와 `sell[1..2]`(t번 매도 완료 최대 이익)의 스냅샷이다.

실제 반환값은 `7` 이며(거래 `[2,6]`으로 +4, `[0,3]`으로 +3), 시뮬레이션 마지막 프레임의 `max(sell)`과 일치한다.
