...
입력 `prices = [7, 1, 5, 3, 6, 4]`에 대해 1회 거래 최대 이익을 구하는 과정이다. `pointers`의 `j`는 현재 매도 후보일, `minLeft`(파랑 인접)는 지금까지의 최저 매수가 인덱스다. `highlight`(빨강)는 현재 검사 중인 `prices[j]`, `marked`(회색)는 최적 매수일이다. keyValue 패널은 `minLeft`(최저가)와 `best`(최대 이익)다.

실제 반환값은 `5` 이며(1일 매수 가격 `1`, 4일 매도 가격 `6`), 시뮬레이션 마지막 프레임의 `best`와 일치한다.
