...
console.log(
  `lcsNaive("abcd","wxyz") = ${naiveResult}, callCount = ${callCount}`,
);
...
console.log("\n=== 점검 문제용: s='abc', t='bca' ===");
console.log(
  `longestCommonSubsequence("abc","bca") = ${longestCommonSubsequence("abc", "bca")}`,
);
{
  const s = "abc",
    t = "bca";
...
console.log("\n=== 함정 코드(대각선 오용) 비교 ===");
console.log(
  `정답 구현 결과: ${longestCommonSubsequenceBasic("abcde", "ace")}, 오답 구현 결과: ${longestCommonSubsequenceWrong("abcde", "ace")}`,
);
{
  ...
  for (let i = 1; i <= n; i++) {
    for (let j = 1; j <= m; j++) {
      dp[i]![j] =
        s[i - 1] === t[j - 1] ? dp[i - 1]![j - 1]! + 1 : dp[i - 1]![j - 1]!;
    }
  }
...
console.log(
  `정답 롤링 결과: ${longestCommonSubsequence("abcde", "ace")}, 오답 롤링 결과: ${longestCommonSubsequenceRolledWrong("abcde", "ace")}`,
);
console.log(
  `정답 롤링(AGGTAB/GXTXAYB): ${longestCommonSubsequence("AGGTAB", "GXTXAYB")}, 오답 롤링(AGGTAB/GXTXAYB): ${longestCommonSubsequenceRolledWrong("AGGTAB", "GXTXAYB")}`,
);
...
for (let iter = 0; iter < 2000; iter++) {
  ...
  if (correct !== wrong) {
    foundDiff++;
    if (foundDiff <= 5)
      console.log(`DIFF s=${s} t=${t} correct=${correct} wrong=${wrong}`);
  }
}
...
console.log("\n=== 롤링 함정 상세 트레이스: s='bab', t='aaaa' ===");
{
  ...
  console.log(
    `오답(rolled wrong): ${longestCommonSubsequenceRolledWrong(s, t)}`,
  );
  ...
  for (let i = 1; i <= n; i++) {
    ...
    console.log(
      `오답 i=${i} (s[${i - 1}]='${s[i - 1]}') 이후 dp = [${dp.join(", ")}]`,
    );
  }
}
...
console.log(
  `정답: ${longestCommonSubsequence("babb", "bbab")}, 오답(롤링 대각선 미보존): ${longestCommonSubsequenceRolledWrong("babb", "bbab")}`,
);
