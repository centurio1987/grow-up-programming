...
입력 `A = [1, 3, 5, 7]`에 대해 BIT를 구성한 뒤 `ops = [query(1,3), update(i=1,v=10), query(1,3)]`을 처리하는 과정이다. `array` 패널은 1-indexed BIT 배열(인덱스 0은 미사용), `highlight`(빨강)는 현재 갱신/조회 중 접근하는 셀, `keyValue` 패널은 논리 배열 `A`와 진행 중인 누적합 스냅샷이다.

실제 반환값은 `[15, 22]` 이며(첫 query는 `3+5+7=15`, update로 `A[1]`이 3→10이 된 뒤 두 번째 query는 `10+5+7=22`), 시뮬레이션 마지막 프레임과 일치한다.
