...
먼저 잔여 그래프를 만들고 Dinic's로 최대 유량을 구합니다. Dinic's 자체의 상세 설명(레벨 배열, 차단 유량, `iter` 포인터의 역할)은 [maxFlow 가이드](../maxFlow/maxFlow-guide.md)에서 다루므로, 여기서는 컷 문제에 맞춰 그대로 재사용한다는 것만 확인하고 넘어갈게요.
