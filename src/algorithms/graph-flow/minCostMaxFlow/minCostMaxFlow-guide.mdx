...
방향 네트워크 `n = 4`, `edges = [[0,1,2,1], [0,2,2,3], [1,3,2,1], [2,3,2,1], [1,2,1,1]]`(각 간선은 `[u, v, 용량, 단위비용]`), `source = 0`, `sink = 3`에 대해 SSP(Successive Shortest Paths)로 최소 비용 최대 유량을 구하는 과정이다. 그래프 패널의 간선 가중치는 현재 잔여 용량, `activeEdge`(빨강)는 현재 증가 경로의 간선, `nodeValue`는 SPFA 최단 비용 거리 dist이다. keyValue 패널은 누적 flow와 cost 스냅샷이다.

실제 반환값은 `{ flow: 4, cost: 12 }` 이며, 시뮬레이션 마지막 프레임의 totalFlow/totalCost와 일치한다.
