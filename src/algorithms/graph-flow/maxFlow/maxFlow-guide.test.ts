 ...
 *
 * 원본 `src/algorithms/graph-flow/maxFlow/maxFlow.test.ts` 는 학습자 스텁을 가져오므로 그대로
...
const CASES: [string, number, Edge[], number, number, number][] = [
  ...
  ["소스와 싱크 사이 경로 없음", 3, [[0, 1, 10]], 0, 2, 0],
  ["간선 없음", 2, [], 0, 1, 0],
  ["소스와 싱크가 직접 이어져 있다", 2, [[0, 1, 100]], 0, 1, 100],
  [
    "용량 0 인 간선은 유량을 안 보탠다",
    3,
    [
      [0, 1, 0],
      [0, 2, 5],
...
for (const [name, n, edges, source, sink, want] of CASES) {
  test(`정본 — ${name}`, () => {
    expect(maxFlow(n, edges, source, sink)).toEqual({ flow: want });
  });
}
...
test("격자 네트워크 — 제약 규모에서도 답이 정확하다", () => {
  ...
  expect(maxFlow(V, edges, 0, 1)).toEqual({ flow: 100 });
});

test("최악을 만드는 계단 입력도 유량이 정확하다", () => {
