...
방향 네트워크 `n = 4`, `edges = [[0,1,3], [0,2,2], [1,2,1], [1,3,2], [2,3,3]]`, `source = 0`, `sink = 3`에 대해 Dinic's로 최대 유량을 구하는 과정이다. 그래프 패널의 간선 가중치는 현재 잔여 용량이고, `activeEdge`(빨강)는 현재 증가 경로에서 유량을 흘리는 간선, `nodeValue`는 BFS 레벨이다. keyValue 패널은 누적 유량과 현재 증가 경로 스냅샷이다.

실제 반환값은 `{ flow: 5 }` 이며, 시뮬레이션 마지막 프레임의 totalFlow와 일치한다.
