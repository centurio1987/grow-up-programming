...
**핵심 아이디어**: "스위프 라인 위에서 인접한 선분끼리만 교차할 수 있다 — 근처에 없는 쌍은 비교조차 하지 않는다."

브루트 포스는 $n$개의 선분에서 모든 쌍을 비교하지만, 실제로 두 선분이 교차하려면 교차 직전에 스위프 라인 위에서 반드시 이웃해 있어야 한다. Bentley-Ottmann은 이 사실을 이용해, 선분의 시작·끝·교차 세 가지 이벤트가 발생할 때만 인접 쌍을 검사한다. 덕분에 멀리 있는 쌍은 아예 무시하고 $O((n+k)\log n)$으로 처리할 수 있다.

**풀이 구조**
1. 각 선분의 왼쪽 끝(START)과 오른쪽 끝(END) 이벤트를 우선순위 큐에 삽입한다.
2. 이벤트를 $x$ 좌표 순으로 꺼내며 스위프 라인을 이동한다.
3. START: BST에 선분 삽입 후 위·아래 이웃과 교차 검사 → 교차 이벤트 추가.
4. END: 선분 제거 후 새로 인접해진 두 이웃 사이의 교차 검사.
5. INTERSECT: 교차 쌍 카운트, BST에서 두 선분의 순서를 swap, 새 이웃 교차 검사.

**조건**: 선분들이 일반 위치에 있거나, 수직 선분과 동시 교차점에 대한 특수 처리가 구현되어 있어야 한다.

**대표 예시**: 도로망 선분 교차 횟수 계산
$n$개의 도로 선분 중 실제로 교차하는 쌍을 셀 때, 브루트 포스의 $O(n^2)$ 대신 스위프 라인으로 $O((n+k)\log n)$에 처리한다. 스위프가 진행될수록 BST는 현재 통과 중인 선분만 관리하므로 비교 횟수가 크게 줄어든다.

**언제 쓰나**
"선분 집합에서 교차하는 쌍의 수를 구하라" 또는 "교차점의 좌표를 모두 열거하라"는 문제에서, 선분 수가 수만 이상이라 $O(n^2)$이 시간 초과일 때 사용한다.
...
예시 선분 3개에 대해 스위프 라인을 실행해 교차 쌍 수를 센다.
`s0 = (0,0)-(6,6)`, `s1 = (0,3)-(6,0)`, `s2 = (0,5)-(6,4)`.
실제 교차는 `s0-s1`(점 (2,2))과 `s0-s2`(점 약 (4.29,4.29)) 두 쌍이고, `s1-s2`는 만나지 않는다. keyValue 패널은 현재 스위프 x, BST에 든 활성 선분의 아래->위 순서, 누적 `count`를 보여준다.

실제 반환값은 `2` 이며, 시뮬레이션 마지막 프레임의 `count`와 일치한다.
...
export const steps = [
  {
    title: "이벤트 큐 구성",
    detail: "각 선분의 START(x=0), END(x=6) 이벤트를 큐에 넣는다. 동일 x에서는 START < INTERSECT < END.",
    entries: [
      { label: "선분", value: "s0,s1,s2" },
      { label: "BST", value: "(비어 있음)" },
      { label: "count", value: 0 },
    ],
  },
  {
    title: "x=0: START 세 선분 삽입",
    detail: "y 순서로 아래->위: s0(y=0), s1(y=3), s2(y=5). 인접 쌍 검사: s0-s1 교차(x=2)는 큐에 추가, s1-s2는 교차 없음.",
    entries: [
      { label: "sweep_x", value: 0 },
      { label: "BST (아래->위)", value: "[s0, s1, s2]" },
      { label: "큐에 추가", value: "INTERSECT(s0,s1)@x=2" },
      { label: "count", value: 0 },
    ],
  },
  {
    title: "x=2: INTERSECT(s0, s1)",
    detail: "교차 쌍 s0-s1 카운트. count=1. BST에서 s0, s1 순서를 swap.",
    entries: [
      { label: "sweep_x", value: 2 },
      { label: "처리 쌍", value: "s0-s1" },
      { label: "BST (아래->위)", value: "[s1, s0, s2]" },
      { label: "count", value: 1 },
    ],
  },
  {
    title: "swap 후 새 이웃 검사",
    detail: "s0이 s2와 새로 인접. s0-s2 교차(x약4.29) 발견 → 큐에 추가.",
    entries: [
      { label: "새 인접 쌍", value: "s0-s2" },
      { label: "큐에 추가", value: "INTERSECT(s0,s2)@x≈4.29" },
      { label: "count", value: 1 },
    ],
  },
  {
    title: "x≈4.29: INTERSECT(s0, s2)",
    detail: "교차 쌍 s0-s2 카운트. count=2. swap → [s1, s2, s0]. 새 이웃에 교차 없음.",
    entries: [
      { label: "sweep_x", value: "≈4.29" },
      { label: "처리 쌍", value: "s0-s2" },
      { label: "BST (아래->위)", value: "[s1, s2, s0]" },
      { label: "count", value: 2 },
    ],
  },
  {
    title: "x=6: END 세 선분 제거",
    detail: "남은 이벤트는 END뿐. 제거 시 새로 인접해진 쌍에 교차 없음. count 변화 없음.",
    entries: [
      { label: "sweep_x", value: 6 },
      { label: "BST (아래->위)", value: "[] (모두 제거)" },
      { label: "count", value: 2 },
    ],
  },
  {
    title: "완료: count = 2",
    detail: "큐가 비어 종료. 교차하는 선분 쌍은 s0-s1, s0-s2 두 쌍.",
    entries: [
