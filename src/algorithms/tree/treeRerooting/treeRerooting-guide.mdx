...
선형 체인 트리 `n = 5`, `edges = [[0,1], [1,2], [2,3], [3,4]]`에 대해 Rerooting으로 각 정점의 거리 합을 구하는 과정이다. 트리 패널(루트 0으로 그림)의 색은 현재 점화식 적용 대상(active=값을 막 확정한 정점, frontier=다음 전파 대상), keyValue 패널은 size 배열과 S 배열의 스냅샷이다.

실제 반환값은 `[10, 7, 6, 7, 10]` 이며, 시뮬레이션 마지막 프레임의 S 배열과 일치한다.
