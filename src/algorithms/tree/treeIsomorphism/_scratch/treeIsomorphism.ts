...
console.log("예시6(n=2, true 기대)", treeIsomorphism(2, [[0, 1]], [[1, 0]]));
...
console.log("=== 임의 루트(중심 아님) vs 중심 루팅 비교 ===");
{
  ...
  console.log(
    `  T1@center(${centers1[0]}):`,
    encodeStringForm(adj1, centers1[0]!),
  );
  console.log(
    `  T2@center(${centers2[0]}):`,
    encodeStringForm(adj2, centers2[0]!),
  );
}
...
console.log("=== 정렬 생략 시 오탐 재현 ===");
{
  ...
  console.log(
    "정렬 포함 결합:",
    encodeStringForm(adj1, 1),
    "vs",
    encodeStringForm(adj2, 1),
  );
}
...
console.log("=== 정렬 생략 재시도 (자식 모양이 다른 트리) ===");
{
  ...
  console.log(
    "A,B는 같은 트리(간선 나열 순서만 다름). naive(true 기대):",
    isIsomorphicByPermutation(n, edgesA, edgesB),
  );
  console.log("정렬 없이:", encodeNoSort(adjA, 5), "vs", encodeNoSort(adjB, 5));
  console.log(
    "정렬 포함:",
    encodeStringForm(adjA, 5),
    "vs",
    encodeStringForm(adjB, 5),
  );
}
