...
트리 `n = 6`, `edges = [[0,1], [0,2], [1,3], [1,4], [2,5]]`, `root = 0`에 대해 Binary Lifting으로 전처리한 뒤 질의 `lca(3, 5)`를 처리하는 과정이다(LOG=3). 트리 패널의 색은 현재 주목 정점(active=올리는 중인 u/v, frontier=목표 깊이/후보), keyValue 패널은 `anc` 테이블 일부와 현재 u, v 스냅샷이다.

실제 반환값은 `[0]` (질의 `lca(3, 5) = 0`)이며, 시뮬레이션 마지막 프레임의 LCA와 일치한다.
