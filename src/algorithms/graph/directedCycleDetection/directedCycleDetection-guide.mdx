...
예시 유향 그래프 `n = 4`, `edges = [[0,1], [1,2], [2,0], [2,3]]`에 대해 3색 DFS를 실행하는 과정이다. 노드 위 글자는 색(W=WHITE 미방문, G=GRAY 현재 스택, B=BLACK 완료)이다. 빨간색은 GRAY(현재 DFS 경로), 회색은 BLACK이다. `keyValue` 패널은 DFS 스택과 검사 중인 간선을 보여준다. 사이클 `0 → 1 → 2 → 0`이 존재한다.

실제 반환값은 `true` 이며, 시뮬레이션 마지막 프레임에서 back edge `2 → 0`이 발견되는 것과 일치한다.
