 ...
 * | 정확한 유효 자릿수의 최솟값 | 그 구간에서 가장 나쁜 자리의 유효 자릿수. **음수일 수 있다** |
 *
 * **자릿수는 음수를 그대로 낸다.** 2026-09-08 까지는 이 자리가 둘로 갈려 있었다 — 그때 P10 이
 * 본문에서 숫자를 뽑을 때 부호를 안 읽어서, 음수 계수를 내면 본문에 그 값을 그대로 적어 두어도
 * 「실측값이 본문에 없다」로 보고됐기 때문이다. 도구가 부호를 읽게 된 뒤(2026-09-10,
 * `KAN-034.9` `S2`) 하나로 되돌렸다. 갈라 두면 「유효 자릿수 0 · 초과 자릿수 295」가 되어
 * **두 수를 합쳐야 뜻이 서는** 모양이었다.
 *
...
export interface 채점 {
  ...
  처음_넘긴_K: number;
  /**
   * 가장 나쁜 자리의 유효 자릿수. **음수일 수 있다** — 상대 오차가 1 을 넘으면 맞는
   * 자릿수가 하나도 없고, 그 크기가 오차 정수부의 자릿수다(`-295` 는 오차의 정수부가
   * 295 자리라는 뜻이다).
   */
  최소_유효_자릿수: number;
}
