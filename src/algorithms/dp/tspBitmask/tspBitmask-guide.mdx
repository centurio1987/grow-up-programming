...
**핵심 아이디어**: "어떤 순서로 방문했는지가 아니라 어떤 도시 집합을 방문했는지와 지금 어디 있는지만 중요하다 — 그 집합을 비트마스크로 표현한다."

외판원 문제(TSP)는 NP-완전이지만, $n \leq 20$이면 비트마스크 DP로 $O(n^2 \cdot 2^n)$에 해결할 수 있다. "0→1→2→3"과 "0→2→1→3"은 방문 집합 $\{0,1,2,3\}$과 현재 위치 3이 같으므로, 이후 남은 도시를 최적으로 방문하는 비용도 동일하다. 이 상태를 $n$비트 정수로 표현하면 $2^n \times n$개의 상태만으로 모든 경우를 커버할 수 있다.

**풀이 구조**
1. `dp[1<<0][0] = 0`으로 초기화하고 나머지는 `+∞`로 설정한다(도시 0 출발).
2. `mask`를 1부터 FULL까지 오름차순으로 순회한다.
3. 각 유효 상태 `(mask, v)`에서 미방문 도시 `u`로 이동해 `dp[mask|(1<<u)][u]`를 갱신한다.
4. `FULL = (1<<n)-1`에서 `dp[FULL][v] + dist[v][0]`의 최솟값으로 출발지 복귀 비용을 계산한다.
5. 그 최솟값을 반환한다.

**조건**: $n$개의 노드를 모두 방문하고 시작점으로 돌아오는 최소 비용 경로를 구해야 하고, $n \leq 20$ 정도로 비트마스크 상태 $2^n$이 메모리에 들어올 때 적용 가능하다.

**대표 예시**: 도시 3개, `dist=[[0,1,2],[2,0,1],[1,2,0]]`
가능한 경로: 0→1→2→0(비용 1+1+1=3), 0→2→1→0(비용 2+2+2=6). 비트마스크 DP는 $dp[011][1]=1$, $dp[111][2]=2$, 복귀 $dp[111][2]+dist[2][0]=2+1=3$을 자동으로 찾아 3을 반환한다.

**언제 쓰나**
"N개의 노드를 모두 방문하는 최소 비용 경로"(순열 최적화)에서 N이 20 이하일 때, 또는 "방문한 원소의 집합"이 상태의 핵심인 문제에서 비트마스크 DP를 사용한다.
...
예시 3개 도시 `dist = [[0,1,2],[2,0,1],[1,2,0]]`에 대해 비트마스크 DP를 채우는 과정이다. matrix 패널의 행은 방문 집합 `mask`(2진 라벨, 비트 i=도시 i 방문), 열은 현재 위치 `v = 0,1,2`이며 셀 값은 `dp[mask][v]`(미달성은 ∞)다. 빨간 셀은 그 단계에서 갱신된 상태다. 도시 0에서 출발해 모두 방문 후 0으로 복귀하는 최소 비용을 구한다.

실제 반환값은 3 이며 (0→1→2→0 = 1+1+1), 시뮬레이션 마지막 프레임에서 `min(dp[111][v] + dist[v][0])`로 확인된다.
