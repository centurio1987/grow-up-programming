...
function countWaste(n: number) {
  ...
  console.log(
    `n=${n}: mask*v 순회=${totalVIter}, v not-in-mask 스킵=${skippedNotInMask} (${((100 * skippedNotInMask) / totalVIter).toFixed(1)}%), 미도달(Infinity) 스킵=${skippedInfinity} (${((100 * skippedInfinity) / totalVIter).toFixed(1)}%), 실제 처리=${processed} (${((100 * processed) / totalVIter).toFixed(1)}%)`,
  );
}
countWaste(12);
countWaste(16);
