...
function knapsack01Naive(
  ...
  function solve(i: number, remainW: number): number {
    ...
    const w = weights[i]!;
    const take =
      w <= remainW ? values[i]! + solve(i + 1, remainW - w) : -Infinity;
...
function knapsack01_2D_WRONG_offset(
  weights: number[],
  values: number[],
  W: number,
): number {
...
function knapsack01_1D_WRONG_ascending(
  weights: number[],
  values: number[],
  W: number,
): number {
...
console.log(
  "=== 1) 가이드 주입 시뮬 입력 검증: weights=[1,3,4], values=[1,4,5], W=5 ===",
);
{
  const weights = [1, 3, 4];
  const values = [1, 4, 5];
...
);
{
  ...
  console.log(
    "1D 오름차순 (버그) :",
    knapsack01_1D_WRONG_ascending(weights, values, W),
  );
}
...
);
{
  ...
  console.log(
    "2D 오프셋버그(오답):",
    knapsack01_2D_WRONG_offset(weights, values, W),
  );
}
...
console.log("\n=== 6) 엣지 케이스 ===");
{
  ...
  console.log(
    "W=0                    :",
    knapsack01_2D([1, 2, 3], [10, 20, 30], 0),
  );
  console.log(
    "모든 물건이 W보다 무거움:",
    knapsack01_2D([10, 20, 30], [100, 200, 300], 5),
  );
  console.log(
    "물건 1개, 정확히 W=W  :",
    knapsack01_2D([5], [42], 5),
    knapsack01_1D([5], [42], 5),
  );
}
