...
고정 입력 `A = [1, 3, 5, 7, 9]`, `target = 7`에 대해 이진 탐색을 실행하는 과정이다. 배열 위 포인터 `lo`, `mid`, `hi`가 탐색 구간을 나타내며, 회색으로 표시된 칸은 단조성에 의해 제거된(target이 있을 수 없는) 인덱스다. 빨강은 현재 비교 중인 `mid`다.

실제 반환값은 `3` 이며, 시뮬레이션 마지막 프레임의 `mid` 위치와 일치한다.
...
export const steps = [
  {
    title: "초기화",
    detail: "lo = 0, hi = 4. 탐색 구간은 전체 배열. target = 7.",
    array: arr,
    pointers: { lo: 0, hi: 4 },
  },
  {
    title: "mid 계산 (구간 [0, 4])",
    detail: "mid = 0 + ⌊(4 - 0) / 2⌋ = 2. A[2] = 5 와 target = 7 비교.",
    array: arr,
    highlight: [2],
    pointers: { lo: 0, mid: 2, hi: 4 },
  },
  {
    title: "A[mid]=5 < 7 → 왼쪽 절반 제거",
    detail: "단조성에 의해 A[0..2]에는 7이 없다. lo = mid + 1 = 3.",
    array: arr,
    marked: [0, 1, 2],
    pointers: { lo: 3, hi: 4 },
  },
  {
    title: "mid 계산 (구간 [3, 4])",
    detail: "mid = 3 + ⌊(4 - 3) / 2⌋ = 3. A[3] = 7 와 target = 7 비교.",
    array: arr,
    marked: [0, 1, 2],
    highlight: [3],
...
<AlgorithmSimulation view="array" steps={steps} title="이진 탐색: target = 7 찾기" />
