...
**핵심 아이디어**: "2의 보수에서 $-x$는 $x$의 최하위 1비트 이하를 정확히 반전시키기 때문에, $x \,\&\, (-x)$는 그 비트 하나만 남긴 값이 된다."

비트를 하나씩 순회하면 32번이 필요하지만, 2의 보수 표현의 수학적 구조를 이용하면 단 한 번의 AND 연산으로 같은 결과를 낸다. $x$의 최하위 1비트 위치 $p$를 기준으로 보면, $x$와 $-x$는 위치 $p$에서 둘 다 1이고, $p$ 초과에서는 서로 반전되어 AND 결과가 0이 되며, $p$ 미만은 둘 다 0이다. 따라서 AND 결과에는 위치 $p$의 비트만 남는다.

**풀이 구조**
1. 입력 `x`를 받는다.
2. `x & (-x)` 단 한 번의 연산으로 최하위 1비트만 남긴 값을 반환한다.

**조건**: 음의 정수가 2의 보수로 표현되어야 한다. 현대 모든 프로세서와 JavaScript 비트 연산이 이를 보장한다.

**대표 예시**: `x = 12 = 0b00001100`
`-12 = 0b11110100`. `12 & (-12) = 0b00000100 = 4`. 위치 $p = 2$의 비트($2^2 = 4$)만 추출된다.
`x = 0`이면 `0 & (-0) = 0 & 0 = 0`으로 자연스럽게 처리된다.

**언제 쓰나**
Fenwick Tree(BIT)에서 인덱스 이동 (`i += i & (-i)`, `i -= i & (-i)`), 비트마스크에서 최하위 비트 제거 (`x &= x - 1`의 반대 연산), 또는 어떤 집합에서 가장 낮은 비트 위치를 $O(1)$로 구해야 할 때 사용한다.
