...
입력 `n = 10` (`0b1010`)에 대해 전이 행렬 `M = [[1,1],[1,0]]`의 거듭제곱을 반복 제곱법으로 계산하는 과정이다. `keyValue` 패널은 남은 `n`의 2진 표현, 현재 비트, `base`(= M의 2^k 제곱), 누적 `result`를 보여준다. 비트가 1인 위치(2^1, 2^3)에서만 `result`에 `base`를 곱한다.

실제 반환값은 `55n` (= F(10), `M^10[0][1]`) 이며, 시뮬레이션 마지막 프레임의 result 행렬과 일치한다.
