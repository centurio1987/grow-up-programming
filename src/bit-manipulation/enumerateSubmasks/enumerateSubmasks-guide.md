...
**핵심 아이디어**: "서브마스크 $s$에서 다음으로 작은 서브마스크는 $(s-1) \,\&\, mask$ 한 번의 비트 연산으로 직접 구할 수 있다."

0부터 mask까지 전체를 순회하면서 서브마스크 조건을 검사하면 $O(mask)$가 걸리지만, 비트 연산의 성질을 이용하면 서브마스크만 정확히 골라 건너뛸 수 있다. $s-1$은 $s$의 최하위 1비트를 0으로 만들고 그 아래를 모두 1로 채우는데, 여기에 `& mask`를 적용하면 mask 밖의 비트가 제거되어 "$s$보다 작은 서브마스크 중 가장 큰 것"이 한 번에 선택된다.

**풀이 구조**
1. 결과 배열을 초기화하고 `s = mask`로 시작한다.
2. `s > 0`인 동안 `s`를 결과에 추가하고, `s = (s - 1) & mask`로 다음 서브마스크로 이동한다.
3. 루프 종료 후 공집합 `0`을 결과에 별도로 추가한다.

**조건**: 입력 `mask`는 음이 아닌 정수여야 한다. `mask = 0`이면 루프가 실행되지 않고 `[0]`만 반환된다.

**대표 예시**: `mask = 0b1011 = 11` (세팅 비트 3개 → 서브마스크 $2^3 = 8$개)
`11 → 10 → 9 → 8 → 3 → 2 → 1 → 0` 순서로 8개의 서브마스크를 중복 없이 내림차순으로 열거한다. 0 ~ 10 범위를 전부 순회(11회)하는 대신 꼭 필요한 8회만 방문한다.

**언제 쓰나**
비트마스크 DP에서 특정 집합의 모든 부분집합을 열거해야 할 때 사용한다. 전체 $2^n$개 mask에 대해 적용하면 합산 복잡도가 $O(3^n)$이며, 이는 이론적 하한과 동일하다.
