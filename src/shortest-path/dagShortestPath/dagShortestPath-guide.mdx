...
예시 DAG `n = 5`, `edges = [[0,1,2], [0,2,6], [1,2,-4], [1,3,1], [2,3,3], [3,4,2]]`, `src = 0`에 대해 위상 순서대로 완화하는 과정이다. 위상 순서는 `[0, 1, 2, 3, 4]`. 노드 위 숫자는 현재 `dist`(∞는 미도달)이다. 빨간색은 현재 처리 중 정점(active), 회색은 처리 완료(visited)이다. 음수 간선 `1→2 (-4)` 덕분에 `dist[2]`가 6에서 -2로 갱신되는 점이 핵심이다.

실제 반환값은 `[0, 2, -2, 1, 3]` 이며, 시뮬레이션 마지막 프레임의 거리 라벨과 일치한다.
