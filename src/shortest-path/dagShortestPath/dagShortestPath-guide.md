...
**핵심 아이디어**: "사이클이 없으니 위상 순서대로 한 번만 훑으면 최단 거리가 확정된다."

음수 간선이 있어도 DAG(비순환 방향 그래프)라면 역방향 간선이 존재하지 않는다. 위상 정렬로 처리 순서를 확정하면, 어떤 정점 $v$를 처리하기 전에 $v$로 들어오는 모든 간선의 출발 정점이 이미 처리되어 있음이 보장된다. 따라서 각 정점을 정확히 한 번만 방문해도 최단 거리가 확정된다.

**풀이 구조**
1. 인접 리스트와 진입 차수(in-degree) 배열을 구성
2. Kahn's 알고리즘으로 위상 순서 계산 (in-degree가 0인 정점부터 큐에 넣어 BFS 방식으로 추출)
3. `dist[src] = 0`, 나머지 Infinity로 초기화
4. 위상 순서대로 각 정점 $u$를 처리: `dist[u]`가 Infinity가 아닌 경우 이웃 $v$들을 완화
5. 완성된 `dist` 배열 반환

**조건**: 그래프가 DAG여야 함. 음수 가중치 허용 (음수 사이클 없음이 DAG에서 보장됨).

**대표 예시**: 프로젝트 일정 최단 완료 시간 계산 (PERT/CPM)
각 작업을 정점, 선행 작업 관계를 간선으로 표현하면 DAG가 된다. 출발 작업에서 각 작업까지의 최장 경로(음수 가중치로 변환)를 구하면 최단 가능 완료 시간을 계산할 수 있다.

**언제 쓰나**
그래프에 사이클이 없고(의존 관계, 일정, 작업 흐름 등) 음수 가중치가 포함된 최단 경로 문제라면 Bellman-Ford($O(VE)$) 대신 이 알고리즘($O(V+E)$)을 사용한다.
