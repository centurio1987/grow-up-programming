...
**핵심 아이디어**: "원점과 각 변으로 이루어진 삼각형들의 부호 있는 넓이를 더하면 다각형 바깥 영역이 자동으로 상쇄된다."

다각형을 삼각분할할 필요 없이 원점을 공통 꼭짓점으로 고정하면, 각 변 $v_i \to v_{i+1}$에 대한 외적 $\frac{1}{2}(x_i y_{i+1} - x_{i+1} y_i)$를 누적하는 것만으로 넓이가 구해진다. 원점이 다각형 밖에 있어도 부호가 반대인 항들이 자동으로 상쇄되어 정확한 넓이가 나온다. 정점을 한 번만 순회하므로 $O(n)$, 추가 메모리는 $O(1)$이다.

**풀이 구조**
1. 누적 변수 `signed_area2 = 0`을 초기화한다.
2. 각 변 $v_i \to v_{(i+1)\%n}$에 대해 $x_i y_{i+1} - x_{i+1} y_i$를 더한다.
3. 루프 완료 후 $|\text{signed\_area2}| / 2$를 반환한다.

**조건**: 자기교차 없는 단순 다각형이어야 한다. 정점 순서(시계/반시계)는 결과에 영향을 주지 않는다(절댓값 처리).

**대표 예시**: 불규칙 토지 면적 측량
경계 좌표 말뚝들의 $(x, y)$ 좌표 목록으로 토지 넓이를 계산하는 상황이다. 삼각분할 등 복잡한 전처리 없이 좌표를 순서대로 한 번만 읽어 $O(n)$에 넓이가 나온다. 오목한 지형도 볼록한 지형과 동일한 코드로 처리된다.

**언제 쓰나**
"다각형의 넓이를 구하라" 또는 Pick 정리 적용을 위해 넓이가 필요할 때 사용한다. 구현이 반복문 하나로 끝나고 정수 좌표에서 오차가 없어, 다각형 넓이가 필요한 모든 기하 문제의 기본 도구로 활용된다.
