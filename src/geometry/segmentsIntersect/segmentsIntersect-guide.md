...
**핵심 아이디어**: "두 선분이 교차하려면 각 선분의 양 끝점이 상대 선분의 직선을 기준으로 서로 반대편에 있어야 한다 — 외적 부호 4개만으로 판별한다."

선분의 교차를 직선 방정식 연립으로 풀면 나눗셈이 필요해 부동소수 오차가 생긴다. 대신 외적(cross product)의 부호만으로 "어느 쪽"을 판단하면 순수 정수 연산이 가능하다. 각 선분의 두 끝점이 상대 선분의 직선을 기준으로 반대편에 있으면 두 직선이 교차하고, 이 교차점이 두 선분 범위 안에 있음도 보장된다. 공선인 경우(외적 = 0)는 bounding box 겹침으로 별도 처리한다.

**풀이 구조**
1. 외적 $d_1 = \text{cross}(p_3, p_4, p_1)$, $d_2 = \text{cross}(p_3, p_4, p_2)$를 계산한다.
2. 외적 $d_3 = \text{cross}(p_1, p_2, p_3)$, $d_4 = \text{cross}(p_1, p_2, p_4)$를 계산한다.
3. $d_1$과 $d_2$가 반대 부호이고 $d_3$과 $d_4$가 반대 부호이면 교차(`true`).
4. 어느 하나라도 0이면 해당 끝점이 상대 선분의 bounding box 안에 있는지 확인한다.
5. 모든 조건을 불만족하면 교차 없음(`false`).

**조건**: 좌표가 정수이거나 수치 오차가 허용되는 범위여야 한다. 부호 비교 시 $d_1 \cdot d_2$의 곱이 오버플로우될 수 있으므로 직접 곱하지 않고 부호를 개별 비교해야 한다.

**대표 예시**: 두 도로 선분의 교차 여부 확인
지도에서 두 도로가 교차하는지 판단하는 상황이다. 끝점 접촉, 겹침 구간, T자 형태 모두 교차로 처리해야 할 때, 외적 4번과 정수 비교만으로 $O(1)$에 정확하게 판별한다. Bentley-Ottmann 등 상위 알고리즘의 핵심 서브루틴으로도 사용된다.

**언제 쓰나**
"두 선분이 교차하는가?"를 정확하고 빠르게 판별해야 할 때, 특히 부동소수 오차를 피하고 싶을 때 사용한다. 단독 쿼리뿐 아니라 Bentley-Ottmann처럼 수많은 선분 쌍을 처리하는 알고리즘의 내부 검사 함수로 활용된다.
