...
**핵심 아이디어**: "점에서 오른쪽으로 광선을 쏘아 다각형 경계와 교차한 횟수가 홀수면 내부, 짝수면 외부다."

Jordan 곡선 정리에 따르면 단순 닫힌 곡선은 평면을 내부와 외부로 분리한다. 임의 방향으로 광선을 쏘면 내부에서 출발한 광선은 경계를 홀수 번 통과하고, 외부에서 출발한 광선은 짝수 번 통과한다. 광선 방향을 $+x$ 수평으로 고정하면 각 변과의 교차 판정이 단순 비교로 줄고, 정점을 정확히 통과하는 경우는 반개구간 처리로 중복 카운트를 방지한다.

**풀이 구조**
1. 점 $p$에서 $+x$ 방향 광선을 정의한다.
2. 각 변에 대해 반개구간 $[\min_y, \max_y)$이 $p_y$를 포함하는지 확인한다.
3. 포함하면 교차점 $x$좌표를 계산해 $p_x$ 이상인지 확인하고 카운트를 증가시킨다.
4. 변 위에 있는지 먼저 검사해 `true`를 즉시 반환한다.
5. `count % 2 == 1`이면 내부, 아니면 외부를 반환한다.

**조건**: 다각형이 자기교차 없는 단순 다각형이어야 한다. 볼록·오목 모두 동작하며 정점 순서(시계/반시계)는 무관하다.

**대표 예시**: 배달 구역 내 주문 판별
지도 위의 불규칙한 배달 구역 다각형과 고객 위치 점이 주어졌을 때, 광선 캐스팅 한 번으로 $O(n)$에 내부 여부를 판정한다. 오목한 구역도 정확히 처리되어 별도 전처리 없이 바로 적용할 수 있다.

**언제 쓰나**
"점이 다각형 내부에 있는가?"를 단순 다각형에 대해 단일 쿼리로 판정할 때 사용한다. 전처리 없이 $O(n)$이며, 볼록 다각형이 아니어서 이진 탐색을 쓸 수 없을 때 특히 유용하다.
