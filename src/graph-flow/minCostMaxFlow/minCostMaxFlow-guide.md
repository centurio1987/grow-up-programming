...
**핵심 아이디어**: "매 단계마다 잔여 그래프에서 가장 저렴한 경로로 유량을 흘리면 전체 비용이 최소가 된다 — 역방향 간선의 음수 비용이 흘린 유량을 되돌리는 환급을 표현한다."

최대 유량을 달성하면서 비용을 최소화하려면, 매 반복에서 소스에서 싱크까지의 최단 비용 경로(SPFA)를 찾아 병목 용량만큼 유량을 흘리면 된다. 이 SSP(Successive Shortest Paths) 전략은 매 단계에서 최단 경로를 선택하지 않았을 때보다 비용이 줄거나 같음이 교환 논증으로 증명된다. 역방향 간선의 비용이 $-a$로 설정되어 있어 유량을 되돌릴 때 원래 지불한 비용을 환급받는 것이 자연스럽게 모델링된다.

**풀이 구조**
1. 각 간선 $(u, v, c, a)$에 정방향(`cost = +a`)과 역방향(`cost = -a`, 용량 0) 쌍을 추가한다.
2. SPFA로 $s$에서 $t$까지 최소 비용 경로를 찾는다.
3. 경로가 없으면 종료하고 결과를 반환한다.
4. 경로 위의 병목 $\delta$를 구하고, 잔여 용량을 갱신한다.
5. `totalFlow += δ`, `totalCost += δ × dist[t]`. 2번으로 돌아간다.

**조건**: 방향 그래프이고 소스 $s \neq$ 싱크 $t$여야 한다. 음수 비용 원래 간선이 없어야 음수 사이클이 발생하지 않는다(SPFA는 음수 가중치는 허용하지만 음수 사이클은 불가).

**대표 예시**: 공장-창고 배송 비용 최소화
$s$개의 공장과 $t$개의 창고, 각 운송로의 용량과 단위 비용이 주어졌을 때 최대 물량을 가장 저렴하게 배송하는 문제다. SPFA가 매번 가장 싼 경로를 선택하고, 역방향 간선이 과거 배송 경로를 취소해 더 나은 조합을 찾을 수 있게 해 전체 비용이 최소화된다.

**언제 쓰나**
"최대 유량을 달성하면서 비용을 최소화하라"는 문제에서 사용한다. 단순 최대 유량에 비용 조건이 추가된 형태이며, 역방향 간선의 음수 비용 때문에 Dijkstra 대신 SPFA를 사용해야 한다.
