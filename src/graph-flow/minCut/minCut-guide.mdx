...
방향 네트워크 `n = 4`, `edges = [[0,1,3], [0,2,2], [1,2,1], [1,3,2], [2,3,3]]`, `source = 0`, `sink = 3`에 대해 Max-Flow Min-Cut으로 최소 컷을 구하는 과정이다. 먼저 Dinic's로 최대 유량을 흘린 뒤(maxFlow 해설과 동일 네트워크), 잔여 그래프에서 s로부터 BFS로 도달 집합 S를 찾고 S→T 원래 간선 용량을 합산한다. 그래프 패널의 간선 가중치는 잔여 용량이고, 노드 색은 S(visited)·T(default)·컷 정점(active), `activeEdge`는 합산 중인 컷 간선이다.

실제 반환값은 `{ cut: 5 }` (최대 유량 5와 동일, 컷 = {s} → 나머지)이며, 시뮬레이션 마지막 프레임의 cutCapacity와 일치한다.
