...
**핵심 아이디어**: "최적 분할점이 단조 증가한다면 — 분할 정복으로 탐색 범위를 반씩 좁혀 $O(kn^2)$을 $O(kn \log n)$으로 줄인다."

연속 구간을 $k$개로 분할하는 DP에서, 비용 함수가 사각 부등식(Quadrangle Inequality)을 만족하면 최적 분할점이 $i$가 증가할수록 단조 비감소임이 보장된다. 이 단조성을 이용해 중앙 인덱스의 최적점을 먼저 구하고, 좌우 재귀 범위를 그 점 기준으로 분리하면 각 계층의 탐색 총량이 $O(n \log n)$으로 감소한다.

**풀이 구조**
1. 기저 계층($g=1$)을 초기화하고, 계층 $g = 2, \ldots, k$ 순서로 처리한다.
2. 각 계층에서 `solve(g, lo, hi, optLo, optHi)`를 호출해 구간 $[lo, hi]$의 최적점을 분할 정복으로 구한다.
3. 중앙 인덱스 `mid`의 최적점을 선형 탐색으로 확정하고, 좌측은 $[\text{optLo}, \text{opt}_{mid}]$, 우측은 $[\text{opt}_{mid}, \text{optHi}]$ 범위로 재귀한다.

**조건**: 비용 함수가 사각 부등식을 만족해야 한다. 이 조건이 없으면 최적점 단조성이 보장되지 않아 잘못된 결과가 나온다.

**대표 예시**: $k$개 그룹 최소 비용 분할
$[0, n-1]$을 정확히 $k$개의 연속 구간으로 나눌 때 총 비용을 최소화하는 문제. 비용 행렬 $\text{cost}[i][j]$가 사각 부등식을 만족하면 분할 정복 DP가 적용된다. 각 계층마다 분할 정복 탐색이 $O(n \log n)$에 수행되고, $k$ 계층의 합이 $O(kn \log n)$이 된다.

**언제 쓰나**
구간 DP의 점화식이 $dp[g][i] = \min_j(dp[g-1][j] + \text{cost}[j+1][i])$ 형태이고, 비용 행렬이 사각 부등식을 만족한다고 알려진 경우에 적용한다.
