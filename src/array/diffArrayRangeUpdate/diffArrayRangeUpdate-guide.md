...
**핵심 아이디어**: "구간 갱신을 두 개의 경계 이벤트로 기록하고, 마지막에 누적합 한 번으로 복원한다."

구간 [l, r]에 값 v를 더하는 연산을 배열 전체에 직접 반영하는 대신, 차분 배열 D에 D[l]+=v, D[r+1]-=v만 기록한다. 모든 갱신이 끝난 후 D의 누적합을 취하면 실제 배열이 한 번에 복원된다. 각 갱신은 O(1), 복원은 O(N)으로 전체 O(N+Q)가 된다.

**풀이 구조**
1. 길이 N+1의 차분 배열 D를 0으로 초기화한다.
2. 각 갱신 (l, r, v)에 대해 `D[l] += v`, `D[r+1] -= v`를 기록한다.
3. D의 누적합을 계산해 실제 배열 A를 복원한다.
4. A를 반환한다.

**조건**: 갱신 후 전체 배열을 한 번에 조회하는 경우에만 효율적. 중간에 특정 인덱스를 조회해야 한다면 갱신할 때마다 누적합을 재계산해야 한다.

**대표 예시**: `N=5, updates=[[1,3,2],[0,2,-1]]`
D = [0,0,0,0,0,0]에 첫 갱신 후 D = [-1,3,0,-1,0,0], 두 갱신 후 D = [-1,3,-1,-1,0,-2]가 되고, 누적합을 취하면 A = [-1, 2, 1, 0, 0]이 된다.

**언제 쓰나**
"구간에 값을 더하거나 빼는 갱신을 여러 번 수행하고, 마지막에 전체 배열 상태를 확인"하는 패턴에서 사용한다. 갱신이 많고 조회가 마지막 한 번뿐일 때 가장 효과적이다.
