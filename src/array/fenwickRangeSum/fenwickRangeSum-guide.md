...
**핵심 아이디어**: "인덱스의 최하위 비트(LSB)가 담당 구간의 길이를 결정해, 갱신과 구간 합 질의를 모두 O(log N)에 처리한다."

Fenwick Tree(Binary Indexed Tree)는 배열 인덱스를 이진수로 바라보는 자료구조다. BIT[i]는 길이 LSB(i)인 구간의 합을 저장하고, 접두어 합 계산 시에는 LSB를 빼가며 O(log N)번 더하고, 갱신 시에는 LSB를 더해가며 O(log N)번 갱신한다. 누적합 배열보다 갱신이 빠르고, 단순 배열보다 질의가 빠른 균형점이다.

**풀이 구조**
1. 크기 N+1의 BIT 배열을 0으로 초기화하고, 초기 배열 A의 각 원소로 `update`를 호출한다.
2. `update(i, delta)`: i에서 LSB를 더해가며 N 이하인 동안 BIT[i] += delta를 반복한다.
3. `prefix(r)`: r에서 LSB를 빼가며 r>0인 동안 합산하면 A[0..r-1]의 합이 된다.
4. 구간 합 `query(l, r) = prefix(r+1) - prefix(l)`을 반환한다.

**조건**: 점 갱신(단일 원소 변경)과 구간 합 질의가 섞여 있는 동적 환경. 구간 갱신이 필요하면 차분 배열과 조합해야 한다.

**대표 예시**: `A=[1,3,5,7]`에서 `query(1,3)=15`
BIT를 구성하면 BIT[4]가 전체 합 16을 저장하고, prefix(4)-prefix(1)로 3+5+7=15를 O(log N)에 계산한다.

**언제 쓰나**
배열 원소가 자주 바뀌면서 구간 합을 반복 조회해야 하는 문제에서 사용한다. 세그먼트 트리보다 구현이 단순하고 상수 인자가 작아, 점 갱신 + 구간 합 질의 조합에서는 Fenwick Tree가 1순위 선택이다.
