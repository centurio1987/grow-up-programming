# 구간 최솟값 질의 (동적 갱신, Segment Tree)

## 함수 인터페이스

```ts
export type SegOp =
  | { type: "update"; i: number; v: number }
  | { type: "query"; l: number; r: number };

export function segmentTreeRangeMin(A: number[], ops: SegOp[]): number[];
```

## 제약 조건

- $1 \leq N \leq 100{,}000$ (여기서 $N$ 은 `A` 의 길이)
- $1 \leq Q \leq 100{,}000$ (여기서 $Q$ 는 `ops` 의 길이)
- $-10^9 \leq A[i], v \leq 10^9$ (정수)
- `update` 연산은 $0 \leq i < N$
- `query` 연산은 $0 \leq l \leq r < N$

## 문제 상세

정수 배열 $A$ 와 일련의 연산이 주어진다. 연산은 두 종류이다.

- **update** $(i, v)$: $A[i] \leftarrow v$ 로 갱신한다 (덮어쓰기).
- **query** $(l, r)$: $\min(A[l], A[l+1], \ldots, A[r])$ 를 계산한다.

모든 연산을 순서대로 처리하며, **`query` 연산의 결과만** 등장 순서대로 담은 배열을 반환한다.

## 예시

```ts
segmentTreeRangeMin(
  [5, 2, 4, 1, 3],
  [
    { type: "query",  l: 0, r: 4 },   // 1
    { type: "query",  l: 0, r: 2 },   // 2
    { type: "update", i: 3, v: 10 },  // A = [5, 2, 4, 10, 3]
    { type: "query",  l: 0, r: 4 },   // 2
    { type: "query",  l: 3, r: 4 },   // 3
    { type: "update", i: 1, v: -7 },  // A = [5, -7, 4, 10, 3]
    { type: "query",  l: 0, r: 4 },   // -7
  ],
);
// [1, 2, 2, 3, -7]
```
