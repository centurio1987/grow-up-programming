...
입력 `nums = [1, -1, 1]`, `k = 1`에 대해 누적합 + 해시맵으로 합이 `k`인 부분 배열을 세는 과정이다. `highlight`(빨강)는 현재 처리 중인 원소 `x`다. keyValue 패널은 누적합 `prefix`, 조회 키 `prefix - k`, 그 키의 맵 값, 누적 `count`, 그리고 맵의 현재 스냅샷을 보여준다(조회 후 기록 순서).

실제 반환값은 `3` 이며(합이 1인 구간 `[0,0]`, `[0,2]`, `[2,2]`), 시뮬레이션 마지막 프레임의 `count`와 일치한다.
