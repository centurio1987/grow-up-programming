...
입력 `A = [-2, 1, -3, 4, -1, 2, 1, -5, 4]`에 대해 Kadane을 실행하는 과정이다. `highlight`(빨강)는 현재 검사 중인 원소 `A[i]`, `marked`(회색)는 현재 `best`를 만든 최적 구간이다. keyValue 패널은 롤링 변수 `prev`(위치 i에서 끝나는 최대합)와 전역 `best`의 스냅샷이다.

실제 반환값은 `6` 이며(구간 `[3..6]`의 합 `4+(-1)+2+1`), 시뮬레이션 마지막 프레임의 `best`와 일치한다.
