...
**핵심 아이디어**: "회전된 배열은 mid를 기준으로 반드시 한쪽 절반이 완전히 정렬되어 있고, 그 절반에 target이 있는지를 끝값 비교만으로 즉시 판정할 수 있다."

배열이 회전되면 전체 단조성이 깨지지만, 회전점은 단 하나뿐이다. 그래서 mid로 나눈 두 절반 중 적어도 하나는 반드시 연속적으로 정렬된 상태다. 정렬된 절반의 양 끝값을 보면 target의 존재 여부를 $O(1)$로 확인할 수 있어, 이 정보를 활용해 이진 탐색을 그대로 적용한다.

**풀이 구조**
1. `[lo, hi]`를 초기 전체 구간으로 설정한다.
2. `mid`를 계산하고 `A[mid] == target`이면 즉시 반환한다.
3. `A[lo] <= A[mid]`이면 왼쪽 절반이 정렬됨: target이 `[A[lo], A[mid])` 범위 안이면 오른쪽 제거, 아니면 왼쪽 제거.
4. 그렇지 않으면 오른쪽 절반이 정렬됨: target이 `(A[mid], A[hi]]` 범위 안이면 왼쪽 제거, 아니면 오른쪽 제거.
5. `lo > hi`가 되면 `-1` 반환.

**조건**: 배열의 모든 원소가 서로 달라야 한다. 중복이 있으면 어느 절반이 정렬됐는지 판별이 불확실해진다.

**대표 예시**: `A = [4, 5, 6, 7, 0, 1, 2]`, `target = 0` 탐색
`lo=0, hi=6` → `mid=3`, `A[3]=7`. `A[0]=4 <= A[3]=7`이므로 왼쪽 정렬됨. `target=0`은 `[4,7)` 범위 밖이므로 `lo=4`. `lo=4, hi=6` → `mid=5`, `A[5]=1`. `A[4]=0 <= A[5]=1`이므로 왼쪽 정렬됨. `target=0`은 `[0,1)` 범위 내이므로 `hi=4`. 결국 `lo=hi=4`, `A[4]=0` 반환.

**언제 쓰나**
"정렬된 배열이 임의의 위치에서 회전됐을 때 탐색하라"는 문제 유형에 직접 적용한다. 중복 원소 허용 변형은 최악 $O(N)$이 되므로 별도 처리가 필요하다.
...
고정 입력 `A = [4, 5, 6, 7, 0, 1, 2]`(인덱스 4에서 회전), `target = 0`에 대해 회전 정렬 배열 탐색을 실행하는 과정이다. 포인터 `lo`, `mid`, `hi`가 탐색 구간을 나타내며, 빨강은 현재 검사 중인 `mid`, 회색은 제거된 인덱스다.

실제 반환값은 `4` 이며, 시뮬레이션 마지막 프레임의 `mid`가 가리키는 위치(`A[4] = 0`)와 일치한다.
