...
**핵심 아이디어**: "여러 패턴을 하나의 자동화 기계로 만들면, 텍스트를 딱 한 번만 읽으면서 모든 패턴을 동시에 찾을 수 있다."

패턴이 수십만 개일 때 각각에 KMP를 적용하면 텍스트를 수십만 번 다시 읽어야 한다. Aho-Corasick은 모든 패턴을 하나의 Trie로 합친 뒤, 불일치가 발생해도 이미 매칭된 정보를 버리지 않는 실패 링크를 추가하여 텍스트를 단 한 번만 순회한다. 검색 기계를 미리 만들어두는 대신 실제 검색은 매우 빠르게 처리하는 전략이다.

**풀이 구조**
1. 모든 패턴을 단일 Trie에 삽입한다. (공통 접두사를 공유하도록 압축)
2. BFS 순서로 각 Trie 노드에 실패 링크를 계산한다. (불일치 시 되감지 않고 이동할 위치)
3. 중간 패턴을 빠르게 찾기 위한 딕셔너리 링크를 함께 구축한다.
4. 텍스트를 한 글자씩 읽으며 Trie 안에서 상태를 전이하고, 매칭되는 패턴을 수집한다.

**조건**: 여러 패턴을 하나의 텍스트에서 동시에 검색해야 할 때. 패턴 수가 많고 텍스트가 길수록 효과가 크다.

**대표 예시**: 바이러스 패턴 여러 개를 파일에서 동시에 탐지하기
파일 내용을 딱 한 번 읽으면서 등록된 수만 개의 바이러스 패턴 중 어떤 것이 어느 위치에서 발견되는지 한 번에 보고한다. 패턴마다 개별 탐색하는 것보다 수만 배 빠르다.

**언제 쓰나**
패턴 집합이 고정되어 있고, 단일 텍스트(또는 여러 텍스트)를 반복 검색해야 할 때 쓴다. 특히 패턴들 사이에 접두사-접미사 관계가 있어도 모두 정확히 보고해야 하는 경우에 적합하다.
