...
- [x] [결재 5] 이해 시험 V2 의 물음 문구를 고칠 것인가 — 명세를 고치는 일이라 제가 정하지 않고 올립니다. 지금은 안 고친 상태입니다.
    ...
    > **판정** — 반려
    > - 승인 · 유저 · 2026-08-20 — V2 를 trade-off 프레임으로 개정하는 안을 승인. 명세·원고·자기시험 픽스처까지 반영한다.

    > **의견** — 어떻게 고치겠다는 건지를 서술해라. 그리고 결과를 미리 정해놓고 억지로 그 결과에 맞추려고 하는 것은 심각한 안티 패턴이다. 또한, 알고리즘마다 장단점이 존재하는데, 단적으로 어떤 알고리즘이 낫다라는 명제는 그 자체로 일반화의 오류다. trade-off 관점에서 분석해야 한다.
    > - ai · 2026-08-20 — 개정안을 서술합니다. 원인은 원고가 아니라 명세이고, 고칠 자리는 셋입니다.
    >   **원인** — 현행 V2 의 실질 통과 조건은 「어느 한 축에서든 이 알고리즘이 수치로 이길 것」입니다. quicksort 는 비교 횟수에서 지지만(52 대 49) 보조 칸에서 이겨(0 대 64) 통과했고, knapsack01 은 표 칸에서 이겨(51,051 대 617,552) 통과했습니다. mosAlgorithm 은 이기는 수치 축이 하나도 없어 세 회차 연속 떨어졌습니다 — 정직하게 지는 편만 걸립니다. 원고 셋은 이미 trade-off 로 썼습니다: quicksort 는 "선택은 어느 쪽이 빠른가가 아니라 메모리를 더 쓸 수 있는가, 최악을 감당할 수 있는가"(`pilot/quicksort/quicksort-guide.md:237-238`), knapsack01 은 "고르는 기준은 알고리즘의 우열이 아니라 어느 축이 짧은가"(`pilot/knapsack01/knapsack01-guide.md:388-390`) 로 끝납니다. 뒤처진 것은 명세 한 곳입니다.
    >   **1. V2 물음** (`tools/comprehension.sh:161-162`) — 현행 "왜 이 알고리즘이 이기는지가 같은 입력의 수치로 대조돼 있는가?" 를 이렇게 고칩니다: "같은 입력·같은 계수로 대조되고, **각 설계가 무엇을 얻고 무엇을 잃는지**가 양쪽 다 적혀 있는가? 이 가이드가 가르치는 절차가 그 수치에서 **져도 통과다** — 지는 사실과 그럼에도 이 절차를 쓰는 축이 함께 있으면 된다. 한쪽이 낫다는 **단정만** 있고 그 대가가 없으면 미통과다." 판정 낱말에 `한쪽만 있음` 을 더해 이 실패를 다른 실패와 구분해 보고하게 합니다.
    >   **2. `purpose.alt` 조항** (`SPEC.md:197-211`) — "같은 입력에서 수치로 **눕힌다**. 셋이 다 있어야 충족이다 — 실명 · 같은 입력 · 수치" 에서 「눕힌다」가 승자 프레임의 출처입니다. "같은 입력에서 같은 계수로 **나란히 잰다**. 넷이 다 있어야 충족이다 — 실명 · 같은 입력 · 수치 · **대가**. 이 가이드의 절차가 져도 된다. **뒤집히는 조건**(어떤 입력·제약에서 반대가 되는가)을 적으면 충족이고, 적을 수 없으면 미흡이다" 로 바꿉니다.
    >   **3. 「결과를 미리 정해놓고 맞추기」를 막을 장치 둘** — 그 안티패턴이 실제로 들어오는 문은 **입력 선택**입니다. 지금은 편마다 입력을 자유롭게 고르므로, 떨어진 편이 통과하려면 자기가 이기는 입력으로 갈아치우면 됩니다. ① 명세에 박습니다 — bench 입력은 `deep.build`·`trace` 가 쓰는 입력과 같은 것을 쓰고, 다르면 왜 다른지 본문에 적는다. 시드·크기·생성식은 본문 명시 의무(세 편 모두 이미 적고 있어 원고 변경 없음). ② 회귀 픽스처 — `comprehension.selftest.sh` 에 **가르치는 절차가 수치에서 지는 편**을 넣고 그것이 V2 PASS 로 나오는지 검사합니다. mosAlgorithm 이 그 픽스처입니다. 이 검사가 있으면 V2 를 다시 승자 프레임으로 되돌리는 순간 자기시험이 빨간불이 됩니다.
    >   **4. 원고 셋 전부** — 개정 V2 로 세 편을 다시 판정합니다(기준이 바뀌었으니 이미 통과한 둘도 다시 돌립니다). 문구도 셋 다 손봅니다 — mosAlgorithm `:228` "이 문제에서는 펜윅이 이긴다" → "이 입력에서 연산 수는 펜윅이 적다" + 뒤집히는 조건 한 줄, quicksort `:233`·knapsack01 `:378` 도 같은 손질. 한 편의 문제가 아니라 세 편에 걸린 클래스로 봅니다.
    >   **안 하는 것** — `check-v2.ts` 에 승자 단정 탐지기를 넣지 않습니다. 일반화의 오류는 형식으로 안 잡히고, 낱말 금지 목록으로 잡으면 오탐이 원고를 왜곡합니다 — 그것이 이번과 같은 종류의 실패입니다. 기계는 형식(P1~P10), 모델은 실질(V1~V7) 이라는 분업을 유지합니다.
