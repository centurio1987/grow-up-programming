...
**재작업 — 2026-08-20 검토 반려분**(결재 5 가 승인으로 뒤집혔다 — V2 를 trade-off 프레임으로 개정한다)

반려 사유: V2 가 "왜 이 알고리즘이 이기는지"를 요구해 **정직하게 지는 편만 구조적으로 탈락**하고,
통과하려면 유리한 입력을 고르게 된다(결과를 미리 정해놓고 맞추는 안티패턴). 어떤 알고리즘이
낫다는 단정 자체가 일반화의 오류이므로 대조는 trade-off 로 쓴다.

- [x] `S15` V2 물음과 `purpose.alt` 조항을 trade-off 프레임으로 개정 (B8)
      완료 기준: `comprehension.sh` V2 가 **양방향**(각 설계가 얻는 것·잃는 것)을 요구한다 ·
      **가르치는 절차가 수치에서 져도 통과**임이 물음에 명시된다 · 대가 없는 우열 단정이
      미통과로 규정된다 · `SPEC.md` `purpose.alt` 충족 요건이 넷(실명·같은 입력·수치·**대가**)이고
      **뒤집히는 조건**이 들어간다 · `comprehension.selftest.sh` 통과
- [x] `S16` bench 입력 고정 규칙을 명세에 박는다 (B8)
      완료 기준: bench 입력은 `deep.build`·`trace` 가 쓰는 입력과 같은 것을 쓰고 다르면 사유를
      본문에 적는다는 조항이 `SPEC.md` 에 있다 · 시드·크기·생성식 본문 명시가 의무다 ·
      세 편이 이미 충족함을 원문으로 확인했다(원고 변경 0 이 정상)
- [x] `S17` 자기시험 픽스처 — 지는 편이 V2 를 통과하는가 (B8)
      완료 기준: `comprehension.selftest.sh` 에 **가르치는 절차가 수치에서 지는** 응답 픽스처가 있다 ·
      V2 를 승자 프레임으로 되돌리면 그 픽스처가 실제로 빨간불이 되는 것을 확인했다 ·
      자기시험 전체 통과
- [x] `S18` 원고 셋 문구 손질 + 개정 V2 재판정 (B8)
      완료 기준: `mosAlgorithm:228` · `quicksort:233` · `knapsack01:378` 의 우열 단정을
      **축과 입력이 한정된 사실 진술**로 낮추고 뒤집히는 조건을 적었다 · 세 편을 개정 V2 로
      **전부 재판정**해 통과(기준이 바뀌었으므로 이미 통과한 둘도 다시 돌린다) ·
      `check-v2` P1~P10 통과 · `bun run tools/ci.ts all` 초록
- [x] `S19` 검토 요청서 갱신 + 재제출 (B8)
      완료 기준: `review-init --refresh` 로 1·2 항만 갱신됐다(3·4 항 무변경) ·
      항목 판정 5/5 승인이 그대로 남아 있다 · 카드가 `→ 검토`
...
- 2026-08-20T18:07 · s:4573fa3d — `실행 계획` 섹션 교체
- 2026-08-20T18:07 · s:4573fa3d · S15 doing — 착수
- 2026-08-20T18:08 · s:4573fa3d · S15 done — V2 를 trade-off 프레임으로 개정 — 판정 낱말에 '한쪽만 있음' 추가(절대 규칙 5·파서·출력 형식), 가르치는 절차가 수치에서 져도 통과임을 물음에 명시. SPEC purpose.alt 충족 요건 넷(실명·같은 입력·수치·대가)+뒤집히는 조건. 자기시험 7항목 통과.
- 2026-08-20T18:09 · s:4573fa3d · S16 doing — 착수
- 2026-08-20T18:13 · s:4573fa3d · S16 done — L20 신설 — bench 입력을 결과에 맞춰 고르지 않는다(trace 와 같은 입력, 다르면 사유·시드·크기·생성식 본문 명시, 사후 교체는 이력에). 전제 하나가 틀렸다: 세 편 다 전개 입력과 bench 입력이 달랐고 사유가 없었다. 전개 입력으로 실측해 사유를 사실로 적었다 — quicksort 4대5(안 갈림) · knapsack 40대108(2.7배, 제약 반영 못함) · mos 11대25(방향이 반대). mos 는 표를 입력 2행으로 바꿔 뒤집힘을 그대로 보이게 했다.
- 2026-08-20T18:13 · s:4573fa3d · S17 doing — 착수
- 2026-08-20T18:15 · s:4573fa3d · S17 done — 자기시험에 되돌림 방지 장치 넷 추가 — ① '한쪽만 있음' 응답이 미통과로 읽히고 표에 남는가 ② V2 가 양방향을 요구하는가 ③ '져도 통과' 를 못 박는가 ④ 우열을 묻지 않는가. V2 를 옛 승자 프레임으로 되돌려 실제로 3건 FAIL 이 뜨는 것을 확인하고 복원했다. 항목 수를 손으로 세지 않게 카운터로 바꿈 — 17항목 통과.
- 2026-08-20T18:15 · s:4573fa3d · S18 doing — 착수
