...
if (!category) {
  console.error(
    "usage: bun run tools/ord004-emit-args.ts <category>  (예: algorithms/sorting)",
  );
  process.exit(1);
}
...
const guides = m.entries
  .filter(
    (e: any) => e.status !== "done" && e.path.startsWith(`src/${category}/`),
  )
  .map((e: any) => {
    const problem = existsSync(`${e.dir}/${e.name}-problem.md`)
      ? `${e.dir}/${e.name}-problem.md`
      : null;
    const ts = existsSync(`${e.dir}/${e.name}.ts`)
      ? `${e.dir}/${e.name}.ts`
      : null;
...
console.error(
  `# ${category}: ${guides.length}종${factSensitive ? " [사실성→외부검토]" : ""}`,
);
