...
export const METAPHORS: { re: RegExp; label: string }[] = [
  {
    ...
    re: /(?<![가-힣])(?:싸|쌌|쌉)|비싸|값싸/, // 어간 — 「싸게」가 샜다
    label: '비용을 값으로 말한 은유 — "비용이 작다/크다" 로',
  },
  {
    // 어간 `이기` 는 계사 「…이기도」와 **같은 글자**라 목적격 조사 뒤에서만 본다.
    re: /(?<=[을를]\s)이기|이긴|이겼|이겨|승자|패자|지는\s*쪽|겨루|겨룬|겨뤄/,
  ...
  },
  {
    ...
    re: /무너|터[지진져질졌]|(?<![가-힣])(?:죽[는고어을면었]|샌다|샌\s|새[어는고]|샜)/,
  ...
  },
  {
    ...
    re: /(?<![가-힣])(?:도는|돈다|돕니다|돌면)|(?<!되)돌[리린]|(?<!되)돌려(?![주준줍줘줬줄])/,
