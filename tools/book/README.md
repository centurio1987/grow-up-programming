...
| **디자인** | Claude Design 「쉽게 이해하는 알고리즘 디자인 템플릿」 — 출처는 `book.config.json` 의 `design.source` |
| 조판 규칙(템플릿을 옮긴 것) | `tools/book/print-css.ts` |
| 디자인 조절값 · 표지 문구 | `book.config.json` 의 `design` — 지면색 · 표식색 · 시리즈 표기 · 뒤표지 문장 · READER · ISBN |

## 디자인

모양의 정본은 Claude Design 의 템플릿이다(표지 · 목차 · 챕터 간지 · 본문 4쪽 · 뒤표지 8쪽).
빌더는 그 수치를 모든 장에 걸리는 규칙으로 옮겼다. 템플릿은 쪽마다 손으로 배치한 견본이고 책은
흐르는 원고라, 옮기는 자리에서 셋이 갈린다.

| 템플릿 | 책 |
| --- | --- |
| 쪽 요소(머리말 · 쪽번호)가 본문 안에 있다 | CSS `@page` 여백 상자가 넣는다. 장마다 이름 붙은 쪽 `bk-ep-N` 으로 「EP. N」 |
| 간지 · 표지 · 뒤표지의 예시 글(배낭 문제 · 가짜 바코드) | 원고와 설정의 값. 파트 요약은 원고의 파트 첫 문단, ISBN 은 `design.isbn` 이 비면 자리째 뺀다 |
| 원고에 없는 구조(파일 이름 · FIG 번호 · TIME/SPACE 칸 · 요약 3칸) | 옮기지 않았다 — 원고가 그 값을 갖고 있지 않다 |

CSS 만으로 못 세우는 구조 셋(간지 · 코드 머리 줄 · 멈춤 상자)은 조각을 만들 때 `fragment.ts` 가
세운다. 서체는 `@fontsource/ibm-plex-sans-kr` · `@fontsource/ibm-plex-mono`(개발 의존성)를
`file://` 로 싣고, 하나라도 못 읽으면 인쇄기가 멈춘다 — 시스템 글꼴로 조용히 찍히지 않는다.
...
3. **쪽수** — 편을 낱장으로 인쇄해 쪽수를 잰다(간지 한 쪽 포함). 챕터마다 강제 쪽 나눔이 걸려 있어 **낱장
   쪽수와 합본에서의 쪽수가 같다**(36 + 47 = 83 으로 실측). 안 바뀐 편은 다시 재지 않는다.
4. **목차** — 잰 쪽수를 누적해 쪽번호를 채운다. 앞붙이 쪽수가 목차 길이에 달려 있어 최대
   세 바퀴 수렴시킨다.
5. **인쇄** — 모든 권을 먼저 잰 뒤(뒤표지가 세 권의 쪽수를 싣는다) 권마다 합본을 **한 번** 인쇄한다. 낱장 PDF 를 이어 붙이면 폰트가 편마다 실려 150MB
...
- **머리말 · 쪽번호는 CSS 가 넣는다.** 크롬 131 부터 `@page` 여백 상자를 구현한다(152 실측).
  표지 · 간지 · 뒤표지는 이름 붙은 쪽 `bk-full`(여백 0 · 머리말 없음)이다. 쪽 크기도 CSS 가 정하고
  (`preferCSSPageSize`), `chrome.ts` 는 서체 로드와 도식 맞춤이 끝나기를 기다렸다 찍는다.
- **도식은 넘치면 글자를 줄인다.** 줄 맞춤이 내용이라 접지 않는다. 인쇄 직전 스크립트(`FIT_JS`)가
  실제 폭을 재서 비율대로 줄이고, 6.5pt 아래로 가야 하면 그때만 접는다.
- **쪽수 캐시는 조판 CSS 까지 같을 때만 믿는다.** 디자인을 고치면 모든 편을 다시 잰다.
...
디자인 의뢰에 쓴 샘플이다. 지금은 반영한 디자인이 실제 원고에서 어떻게 서는지 보는 용도로 쓴다.
세 권 원본은 합쳐 3천 쪽 가까이 되어 통째로 넘길 수 없다. `build-sample.ts` 가
