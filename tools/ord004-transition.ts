...
// 이름은 중복될 수 있다(예: ahoCorasick 가 algorithms/string 와 data-structures/trie 양쪽에 존재).
...
if (!entry) {
  if (named.length > 1) {
    console.error(
      `'${name}' 항목이 ${named.length}개 있는데 신규파일 디렉터리(${newDir})와 일치하는 것이 없습니다:\n` +
        named.map((e: any) => `  - ${e.path}`).join("\n"),
    );
    process.exit(1);
  }
  entry = named[0];
