...
> · 현재 집필 방식은 마지막의 **2026-09-21 집필 방식 개정**과 자료구조 v2 명세를 따른다. 이전 단계별 규격은 전환 이력이다.
...
## 2026-09-21 집필 방식 개정 — 독자의 이해 순서와 검증 절차 분리

사용자 요청: 문장 생성 원인 분석을 자료구조 재집필 플랜에 반영하고, 직접 작성한 파일럿을 제시한다. 이번 범위는 생성 지침·관련 검사 수정과 덱 파일럿 한 편이다. 전체 가이드 재생성과 사용자 승인은 이번 결과에 포함하지 않는다.

### 원인과 변경

| 확인한 문제 | 반영할 변경 |
| --- | --- |
| 계약 요약을 중복 정본으로 취급해 연산명과 O 표기를 함께 쓴 표를 금지함 | 요약을 허용하고 출처와 한정자를 명시한다. P18은 명시적으로 표시한 요약표의 연산·상한·한정자를 계약 헤더와 대조한다. 일반 비교표는 금지하지 않는다. |
| 계약이 배제하는 것과 검사 절차가 설명의 출발점이 됨 | 사용할 연산, 작은 상태, 단순한 구현의 비용, 저장 방식의 선택 순으로 설명한다. 계약·계측 정보는 주장에 필요한 자리에서 소개한다. |
| 쉬운 단어로 바꾸다가 대상·조건·결과가 생략됨 | 기술 용어는 필요할 때 정의하고 사용한다. 짧은 대체어보다 실제로 무엇을 읽고 바꾸는지 설명한다. |
| 연산별 검사 단위를 문단 구성에 그대로 적용함 | 구현 원리와 독자의 질문으로 묶고, 초안 뒤에 연산 누락을 점검한다. 묶은 이유를 첫 문장에 쓰라는 요구를 없앤다. |
| 참조 구현 한 가지가 자료구조 전체의 설명 범위를 결정함 | 집필 전에 주요 구현 계열을 확인한다. 덱 파일럿은 두 스택·원형 버퍼·이중 연결 리스트를 모두 다룬다. 구현 개수는 다른 자료구조에 일반화하지 않는다. |
| 정성적 권고가 강제 규칙과 충돌함 | L41·L42·L46 및 관련 본문을 직접 고친다. 기술적 검증은 유지하고 문체 점수·새 금칙어·문장 수 제한은 추가하지 않는다. |

### 실행 순서와 검증

1. 현재 계약과 참조 구현을 읽고, 주요 구현들의 저장 상태·연산 처리·비용 차이를 정리한다.
2. 독자가 상태 변화를 따라갈 수 있는 설명을 작성한 뒤 명세의 절에 배치한다. 정본 관리와 교육적 반복을 구분한다.
3. 계약 요약, 전체 코드, 예시, 시뮬레이션이 같은 API를 사용하는지 대조한다. 코드 추출·실행·복잡도 검증을 수행한다.
4. 요약표 검사의 회귀 시험은 정확한 요약 허용, 잘못된 상한·한정자·연산 거부, 일반 구현 비교표 허용을 확인한다.
5. 덱 파일럿을 HTML로 만들어 본문·목차·시각화·코드 표시를 확인하고 사용자에게 보여 준다. 자동 검사 통과와 문장 품질 판단을 구분한다.

집필 지침의 정본은 [자료구조 v2 명세](../sandbox/ds-guide-v2/SPEC.md)와 [집필 안내](../sandbox/ds-guide-v2/AUTHORING.md)다. 이 개정은 이전 전략의 계약 표 복사 금지와 연산 묶음 도입문 강제보다 우선한다. 알고리즘 명세와 계약·구현 자체는 이번 범위에서 변경하지 않는다.
