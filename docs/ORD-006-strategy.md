...
## KAN-035 파일럿 피드백의 전체 집필 반영

2026-09-14 덱 파일럿 재집필에서 확인한 설명 방식을 이후 자료구조 가이드 작성에도 적용한다. 구체적인 요령은 [자료구조 가이드 집필 안내](../sandbox/ds-guide-v2/AUTHORING.md)에 한 벌만 유지하며, 새 집필과 요청받은 재집필 전에 현재 [v2 명세](../sandbox/ds-guide-v2/SPEC.md)와 함께 읽는다. 위의 8단계·MDX 관련 설명은 이전 전환 전략의 기록이며, v2 집필의 산출 형식은 현재 명세를 따른다.

전체 가이드의 일관성은 자료구조의 연산에서 저장 방식, 상태 변화, 정확성, 비용과 선택 기준으로 이어지는 설명에서 확보한다. 참조 구현을 유일한 설계로 가정하지 않고 주요 구현 계열을 살핀다. 덱의 두 스택·원형 버퍼·이중 연결 리스트가 그 예이며, 과거 진단표의 특정 두 배열 구현에 대한 평가는 두 스택 계열 전체에 대한 배제가 아니다.

집필 안내는 정성적 권고로 사용한다. 문체 점수, 문장 수, 필수 표현, 자동 합격 기준이나 별도의 승인 게이트로 전환하지 않는다. 코드·실행 결과 등 기존의 검증 가능한 항목과 구분하며, 게이트 통과만으로 읽기 좋은 글이라고 결론 내리지 않는다. 파일럿의 추가 피드백은 먼저 집필 안내에 반영하고 이후 작성 범위에 적용한다. 이미 작성된 가이드의 일괄 수정이나 파일럿 승인을 자동으로 의미하지 않는다.

### 메모리를 직접 다뤄야 이득이 생기는 구조는 Rust 구현을 싣는다

2026-09-14 XOR 연결 리스트 파일럿 검토에서 더한 방침이다. 유저 지시 원문은 *"xor 연결 리스트는 메모리를 직접 사용하지 않는 한, 이득을 볼 수 없는 자료구조다. 이런 경우에 한 해서, '수행으로 알아보는 자료구조'에 rust 기준 서술을 추가해야 한다."* 이다.

- **무엇을 하는가.** 그 구조를 쓰는 이유가 메모리 주소·배치를 직접 다룰 때만 생기는 구조는, 가이드의 「수행으로 알아보는 자료구조」에 TypeScript 구현과 함께 Rust 구현을 싣는다. 목록 밖의 구조에는 싣지 않는다.
- **왜 이 방침인가.** TypeScript 구현이 그 이득을 없애는 구조에서는 TypeScript 코드만으로 구조가 존재하는 이유를 보일 수 없다. XOR 연결 리스트의 TypeScript 구현은 원소당 값 자리가 이중 연결 리스트보다 많고(5 대 3, 하한), 주소를 직접 XOR 한 Rust 구현은 노드 하나가 16바이트로 이중 연결 노드의 24바이트보다 작다.
- **규약과의 관계.** 규약4 (나)가 Rust 산출물을 강제하지 않는다는 조항은 그대로다. 이 방침은 등급과 다른 축에서 가이드 서술 의무만 더한다. 규약의 정본 서술은 [`docs/ORD-006-conventions.md`](./ORD-006-conventions.md) §규약4 「메모리를 직접 다뤄야 이득이 생기는 구조」이고, B9 에서 "Rust 로 옮겨도 정직해지지 않는다"고 한 판정을 같은 자리에서 정정했다.
- **강제 지점.** 대상 목록은 `tools/ord006-escalation.ts` 의 `DIRECT_MEMORY`(지금 `linear/xorLinkedList` 하나), 구조 판정은 [v2 명세](../sandbox/ds-guide-v2/SPEC.md) `L47` 과 `tools/check-v2.ts` P20(양방향), 코드는 `rust/structures/src/` 에서 추출하고 `cargo test` 가 축1 vector 를 재생한다. 서술 요령은 [집필 안내](../sandbox/ds-guide-v2/AUTHORING.md) 「메모리를 직접 다뤄야 이득이 생기는 구조는 Rust로도 구현한다」에 둔다.
- **전개 카드에 넘길 것.** 동기가 메모리인데 계약 전환 전이라 판정하지 않은 구조가 다섯이다(`bitArray`·`bloomFilter`·`countMinSketch`·`cuckooFilter`·`hyperLogLog`). 각 편을 전환할 때 TypeScript 안에서 절약을 실제 저장으로 보일 수 있는지 판정하고, 못 보이면 목록에 넣는다.
