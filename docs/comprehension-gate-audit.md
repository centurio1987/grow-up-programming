...
```
algo 111편 — 재집필 완료 2편 (래칫 baseline 178편 남음 = 남은 부채)
  T1 미통과  50편   전면 재집필 (Q1 경쟁 설계 + 전개를 한 번에)
...
| 카테고리 | 편 |
| --- | --- |
| array | `mosAlgorithm`(6) · `diffArrayRangeUpdate`(5) · `houseRobber`(5) · `longestCommonSubsequence`(5) · `slidingWindowMaximum`(5) |
...
exemplar 순환을 피하려고 시험편에서 뺐던 편이다.
...
1. **`mosAlgorithm` v3.0.0 의 H축 판정** — 할당량 회복 후 가장 먼저. 재집필이 실제로
   먹혔는지가 나머지 51편의 재집필 방식을 정한다.
2. **미판정 37편 판정** — 배치 러너 재실행(재개 가능).
3. **ds 트랙 실측** — 위 명세 대조 진단을 게이트로 확인한다. 특히 (b) 가 "있음"으로 잡힌
   10편이 진짜인지.
4. **모범 예시 교체** — 통과 22편 중에서 고른다. `paths.json` 의 `exemplars.algo-guide` 와
   `specs/algo-guide/spec.md` 머리말 두 곳. **H축을 통과한 판본이 나오기 전에는 바꾸지 않는다** —
   통과 22편도 "5개 질문에 답이 가능하다"는 하한만 넘은 것이다.
5. **나머지 51편 재집필** — 다중 실패 6편이 앞선다(`fenwickRangeSum`, `ternarySearch`,
...
- ~~`mosAlgorithm` 재집필~~ — v3.0.0(2026-08-15). H축 판정만 남았다.
