...
```
algo 111편 — 재집필 완료 5편 (래칫 baseline 175편 남음 = 남은 부채)
  T1 미통과  47편   전면 재집필 (Q1 경쟁 설계 + 전개를 한 번에)
...
| 카테고리 | 편 |
| --- | --- |
| array | ~~`mosAlgorithm`(6)~~ · ~~`diffArrayRangeUpdate`(5)~~ · ~~`houseRobber`(5)~~ **셋 완료** · `longestCommonSubsequence`(5) · `slidingWindowMaximum`(5) |
...
2. **외부 모델 재판정** — 시험편 A·B 와 T1-01~03 **다섯 편**이 haiku fallback 판정이다.
   agy 할당량이 먼저 풀리므로(약 113시간) 회복 시 다섯 편을 한 번에 다시 돌린다.
...
6. **나머지 47편 재집필** — 다중 실패 6편이 앞선다(`fenwickRangeSum`, `ternarySearch`,
