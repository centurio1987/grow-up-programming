...
1) 누락/생략: 풀이를 끝까지 이해하는 데 꼭 필요한데 빠진 단계(원천 아이디어→솔루션 유도의 비약, 빠진 경계/예외 케이스, 설명 없는 핵심 용어·기호, 불변식·증명의 누락). "이 유도는 한 단계 더 쪼개야 한다"는 곳을 구체적으로.
   특히 **결론만 있고 이유가 없는 논증**을 빠짐없이 지적해 줘: 부등식을 제시하면서 각 항의 부호 근거를 말하지 않는 곳, 귀납의 기저나 단계가 빠진 곳, "성립해서 / 알 수 있습니다 / 이를 반복 적용하면 / 자명합니다"로 논증을 끝낸 곳, 제약·불변식을 선언만 하고 깨졌을 때 무엇이 망가지는지 구체 반례가 없는 곳.
