 ...
 *
 * 원본 `src/algorithms/sorting/quicksort/quicksort.ts` 와 **같은 절차**다 — 중앙 피벗을
...
function sort(arr: number[], lo: number, hi: number): void {
  ...
  // 중앙을 피벗으로 고르고 끝으로 옮긴다. 그래야 분할 루프가 끝 한 칸만 비켜 두면 된다.
  ...
  // i 는 "작은 값이 놓일 다음 자리". j 는 훑는 자리.
  let i = lo;
  for (let j = lo; j < hi; j++) {
    if ((arr[j] as number) < pivot) {
      // ① 피벗보다 작다 — 작은 쪽 구역으로 보내고 그 구역을 한 칸 넓힌다.
      [arr[i], arr[j]] = [arr[j] as number, arr[i] as number];
      i++;
    }
    // ② 피벗보다 크거나 같다 — 그대로 둔다. j 만 나아간다.
  }

  // 피벗을 두 구역 사이(i)로 되돌린다. 이 자리가 피벗의 최종 위치다.
