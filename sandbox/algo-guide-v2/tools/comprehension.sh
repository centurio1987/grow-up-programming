...
#
# ############################################################################
# **2026-08-29 부로 돌리지 않는다.** 유저가 외부 모델 없이 가기로 정했다.
# 이 스크립트가 재던 것 중 **값이 참인가**는 `tools/check-proof.ts` 로 내려갔고,
# **산문이 설명하는가**는 `FEEDBACK.md` §3 의 사람 목록으로 갔다. 대응은
# `SPEC.md` §0 의 표에 있다.
#
# 지우지 않는 이유는 둘이다 — `verdicts/` 31개가 이 스크립트의 산출이라 그 기록을
# 읽으려면 규격이 남아 있어야 하고, 방침이 되돌아오면 그대로 다시 쓴다.
# 자기시험(`comprehension.selftest.sh`)도 같은 이유로 남긴다.
# ############################################################################
#
...
#
# 여기에 ⑤ 를 더했다 — 견주는 대목에 **한쪽 이점만** 있고 대가가 없으면 `한쪽만 있음`.
# 옛 V2 는 "왜 이 알고리즘이 이기는지" 를 물어서, 어느 축에서도 수치로 못 이기는 편은
# 정직하게 쓸수록 떨어졌고(mosAlgorithm r10~r12) 통과하려면 자기가 이기는 입력을 고르게 됐다.
# 결과를 정해 놓고 입력을 맞추는 것이 이 시험이 막으려던 바로 그 행위다. 2026-08-20 결재 5.
#
# 새로 한 것 넷:
...
5. 두 설계를 **견주는** 대목에서 한쪽의 이점만 있고 그 대가가 없으면 판정은
   **`한쪽만 있음`** 이다. 이것은 통과가 아니다. "A 가 B 보다 낫다" 는 그 자체로 성립하지
   않는다 — **어느 축에서 · 어떤 입력에서 · 무엇을 내주고** 얻는지가 함께 있어야 한다.
...
- **V2** 같은 목표를 노리는 **다른 절차**가 **실명으로** 제시되고, **같은 입력·같은 계수의
  수치로** 대조되며, **각 설계가 무엇을 얻고 무엇을 잃는지가 양쪽 다** 적혀 있는가?
  이 가이드가 가르치는 절차가 그 수치에서 **져도 통과다** — 지는 사실과, 그럼에도 이 절차를
  쓰는 축(일반성·최악 보장·메모리 등)이 함께 있으면 된다. 반대로 이겨도 그 대가가 없으면
  `한쪽만 있음` 이다. **어느 쪽이 우월한가를 묻는 물음이 아니다.**
...
각 질문마다 아래 두 줄을 낸다. 판정은 다섯 낱말 중 하나다 —
`PASS` · `FAIL` · `근거 없음` · `결론만 있음` · `한쪽만 있음`.
...
OK = "PASS"
LINE = re.compile(r"^\s*\[(V[1-7])\]\s*(PASS|FAIL|근거 없음|결론만 있음|한쪽만 있음)", re.M)
